...
- **403_csrf.html** → core error templates
...
- **Display**: Shows language name in user's local script (e.g., "हिन्दी" for Hindi)
...
templates/
├── layouts/
│   └── base.html (NEW UNIFIED BASE)
│       ├── {% load i18n %}
│       ├── Language switcher block
│       ├── Navigation (main_nav.html)
│       ├── Messages/alerts block
│       └── Footer
├── _base.html (DEPRECATED - now extends layouts/base.html)
├── components/
│   └── navigation/
│       └── main_nav.html (ENHANCED with i18n)
└── [115+ other templates]
    └── All now extend 'layouts/base.html'
```
...
- ✅ Faster template loading (unified inheritance chain)

## Backward Compatibility
- ✅ All existing URLs work unchanged
- ✅ All existing views work unchanged
- ✅ Public API & webhooks unaffected
