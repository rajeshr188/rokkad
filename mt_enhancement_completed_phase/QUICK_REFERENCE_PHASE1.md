...
|----------|--------|---------|
| `Dashboard()` | ✅ **UPDATED** | Smart router - redirects to correct destination |
| `company_dashboard()` | ⚠️ **DEPRECATED** | Now redirects to `workspace_dashboard` |
...
pages.Dashboard() [Smart Router]
    ↓
    ├─ Has workspace? → workspace_dashboard(workspace_id) ✅
...
- ✅ `apps/orgs/views.py` - Renamed 10 views, added workspace_dashboard
- ✅ `pages/views.py` - Simplified Dashboard, deprecated company_dashboard
- ✅ `apps/orgs/urls.py` - Restructured with new patterns
- ✅ `pages/urls.py` - Import from orgs, simplified patterns
