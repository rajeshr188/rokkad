...
### ✅ Task 1: Intelligent Landing/Routing Logic
**File**: `pages/views.py`
...
### ✅ Task 3: Subscription Validation Middleware
...
### ✅ Task 4-5: Workspace Management Views & Templates
...
### ✅ Task 6: Invitation Expiration Enforcement
...
└─→ Dashboard (redirects to workspace_home)
    │
    └─→ workspace_home (Intelligent Router)
        │
        ├─→ No memberships? → Create workspace
        │
        ├─→ Has workspace selected? → company_dashboard
        │
        └─→ Multiple workspaces? → user_workspaces selector
            │
            ├─→ Your Workspaces
            │   ├─ Click "Switch" → workspace_select
            │   ├─ Click "Enter" → company_dashboard
            │   └─ Click "Create" → orgs_company_create
            │
            ├─→ Pending Invitations Sidebar
            │   ├─ Click "Accept" → Accept & set as active
            │   └─ Click "Decline" → Stay in selector
            │
            └─→ Quick Actions
...
### 1. **Test User Routing**
- [ ] Login with user having 0 memberships → Should see workspace creation form
- [ ] Login with user having 1 membership → Should redirect to company_dashboard
- [ ] Login with user having 2+ memberships → Should see workspace selector
...
- [ ] Access workspace with active subscription → Should work
- [ ] Disable subscription is_active → Should redirect to billing
- [ ] Check subscription expiring in <7 days → Should show warning
- [ ] Check past-due subscription → Should show error message

### 5. **Test Expiration**
- [ ] Create invitation with manual sent date (7+ days ago)
- [ ] Load user_workspaces → Invitation should not appear
- [ ] Try to accept via direct URL → Should fail
