...
1. **Business Docs** (immutable in terms of accounting): GivenLoan, TakenLoan, SalesInvoiceVoucher, etc. — mutable, subject to updates/corrections
2. **Vouchers** (lifecycle-driven): DRAFT → POSTED → CORRECTED/REVERSED — represent the intent to post to GL
...
The Girvi **LoanFlow** currently defines state transitions on the Loan business doc (CREATED → APPROVED → DISBURSED → RELEASED, etc.), but it's **ambiguous and incomplete** about how those transitions map to voucher lifecycle.
...
- Does APPROVED → DISBURSED automatically create a `LOAN_DISBURSE` voucher?
- Does DISBURSED → RELEASED automatically create a `GIVENLOAN_RELEASE` voucher?
...
- **Option A (Doc-driven)**: Loan status drives voucher creation. Status changes → auto-create/post voucher.
- **Option B (Voucher-driven)**: Voucher posting drives loan status. User posts voucher → auto-update loan status.
...
- CREATED → APPROVED (approval decision, no GL impact)
- APPROVED → DISBURSED (cash goes out, creates voucher and JE)
- DISBURSED → RELEASED (collateral released, may create voucher)
- DISBURSED → DEFAULTED (event recorded, no GL impact... or maybe it does?)
- DEFAULTED → AUCTIONED (event recorded, no GL impact)
...
### Layer 1: Business Doc (Loan) — **Mutable, Version-Tracked**

**Role:** Represent the evolving business contract and operational state of a loan.

**Responsibility:**
- Maintain loan attributes (principal, rate, borrower, etc.) — can change pre-disbursement, or via amendments
...
```
CREATED → APPROVED              [Decision: loan is approved]
APPROVED → DISBURSED            [Event: cash disbursed to borrower]
DISBURSED → RELEASED            [Event: collateral released to borrower]
DISBURSED → DEFAULTED           [Event: loan marked as in default]
DEFAULTED → AUCTIONED           [Event: collateral auctioned]
CREATED, APPROVED → CANCELLED   [Cancel: loan never disbursed]
```
...
### Layer 2: Voucher + JournalEntry — **Immutable Once Posted**
...
**Voucher Lifecycle:**
```
DRAFT → POSTED              [User submits for posting]
POSTED → REVERSED           [User reverses via reversing voucher]
POSTED → CORRECTED          [User corrects via correcting voucher]
```
...
### Layer 3: Flow Transitions — **Liaison Between Doc and Voucher**
...
class LoanFlow(object):
    ...
    TRANSITION_ACCOUNTING_IMPACT = {
        ...
        'disburse': True,           # Creates LOAN_DISBURSE voucher → JE
        'deliver': True,            # Creates GIVENLOAN_RELEASE voucher → JE
...
def disburse(self, disbursed_by):
    ...
    Voucher.objects.create(
        ...
        narration=f"Disbursement of ₹{self.loan.principal_amount} to {self.loan.borrower}",
    )
...
### 3. **Define Voucher ← → Loan Status Sync Rules**
...
| CANCELLED | — | — | Vouchers remain (history) but no new vouchers |
| DEFAULTED | — | — | No automatic voucher; optional memo entry |
...
|---|---|---|---|---|
| **Loan (BusinessDoc)** | Girvi | Yes (pre-disbursal) | Custom (CREATED → APPROVED → ...) | Operational: track approvals, releases, defaults |
| **Voucher** | DEA | No (post-POSTED) | DRAFT → POSTED → REVERSED | Accounting: immutable GL records |
| **JournalEntry** | DEA | No | Posted (immutable) | Ledger: balance calculations |
| **Flows** | Girvi | Trigger-only | Liaison | Bridge: map doc transitions → voucher creation |

**Flows are the liaison only.** They should:
1. Update loan status ✓
2. Create vouchers (in DRAFT) ✓
