...
│   └── views.py       # switch_workspace, profile views
├── pages/             # Public pages + dashboard (MIXED CONCERNS)
│   ├── views.py       # Dashboard, company_dashboard, workspace_home
│   └── templates/pages/
├── apps/
│   ├── onboarding/    # ✅ GOOD: Well-organized onboarding wizard
│   │   ├── views.py   # 4-step wizard: profile → company → team → tour
│   │   └── templates/onboarding/
│   ├── orgs/          # ⚠️ PROBLEM: Mixed with pages/ functionality
│   │   ├── views.py   # Company CRUD, memberships, invitations, workspace_home, workspace_select
│   │   └── templates/company/
│   ├── subscriptions/ # ✅ GOOD: Self-contained subscription logic
│   │   ├── views.py   # Plans, checkout, payment
│   │   └── templates/subscriptions/
│   └── tenant_apps/   # ✅ GOOD: Tenant-specific data apps
│       ├── girvi/
│       ├── dea/
│       ├── sales/
│       └── contact/
└── templates/
    ├── _base.html     # Public base template
    ├── tenant.html    # Tenant base template
    ├── company/       # Workspace/company templates
    ├── pages/         # Public + dashboard templates (MIXED)
...
User logs in
    ↓
pages.views.Dashboard()  ← Decorator: @onboarding_required
    ↓
return redirect('workspace_home')
    ↓
orgs.views.workspace_home()  ← Shows workspace list
    ↓
if user.profile.workspace exists:
    return redirect('company_dashboard')
    ↓
pages.views.company_dashboard()  ← Finally shows dashboard
```
...
**Found Files:**
```
templates/company/workspace_home.html       ← Used by orgs.views.workspace_home
templates/pages/workspace_home.html         ← Unused? Or used by pages?
templates/company/workspace_invitations.html
templates/pages/workspace_invitations.html   ← DUPLICATE
templates/pages/company_dashboard.html
templates/pages/tenant.html                  ← Also serves as dashboard?
```
...
/dashboard/                    ← pages.Dashboard (redirector)
/workspace/home/               ← orgs.workspace_home (selector)
/workspace/dashboard/          ← pages.company_dashboard (actual dashboard)
/orgs/company/list/            ← orgs.company_list
/orgs/company/<id>/            ← orgs.company_detail
/orgs/company/<id>/invite      ← orgs.create_invite
...
1. ✅ **Onboarding** - EXISTS (4-step wizard implemented)
2. ✅ **Workspace Creation** - EXISTS (`company_create` view)
3. ✅ **Invitation Sending** - EXISTS (`create_invite` view)
4. ✅ **Invitation Acceptance** - EXISTS (`workspace_invitations`, `CustomAcceptInvite`)
5. ⚠️ **Team Management Dashboard** - PARTIAL (views exist, but no unified team management page) invitations page)
6. ⚠️ **Workspace Settings** - PARTIAL (preferences exist, but scattered)
7. ⚠️ **Subscription Dashboard** - EXISTS but not integrated into main flow
8. ❌ **Permission-Based UI Components** - MISSING (no template tags for permission checking)
9. ❌ **Role Badge Components** - MISSING (no reusable components)
10. ❌ **Empty State Components** - MISSING (no standardized empty states)
...
## Part 3: What Actually Works Well ✅
...
**Tasks:**
1. Move `pages/views.py::company_dashboard` → `apps/orgs/views.py::workspace_dashboard`
2. Move `pages/templates/pages/company_dashboard.html` → `templates/company/dashboard.html`
3. Keep `pages/views.py::Dashboard` as simple landing router only
4. Rename `workspace_home` → `workspace_list` for clarity
...
templates/
├── layouts/
│   ├── base.html              # Universal base (replaces _base.html)
│   ├── public.html            # Extends base (for public pages)
│   └── workspace.html         # Extends base (replaces tenant.html)
│
├── components/
│   ├── navigation/
│   │   ├── main_nav.html      # Main navigation bar
│   │   ├── sidebar.html       # Workspace sidebar
│   │   └── workspace_switcher.html  # Workspace dropdown
│   ├── team/
│   │   ├── member_card.html   # Team member card
│   │   ├── role_badge.html    # Role badge component
│   │   └── invitation_card.html
│   ├── workspace/
│   │   ├── card.html          # Workspace card
│   │   └── stats.html         # Workspace stats widget
│   ├── permissions/
│   │   ├── action_buttons.html  # Permission-gated buttons
│   │   └── feature_gate.html    # Feature availability check
│   └── common/
│       ├── empty_state.html   # Empty state component
│       ├── loading.html       # Loading spinner
│       └── alert.html         # Alert/message component
│
├── workspace/                 # Workspace app templates (was company/)
│   ├── list.html             # List workspaces
│   ├── dashboard.html        # Main workspace dashboard
│   ├── detail.html           # Workspace details
│   ├── form.html             # Create/edit workspace
│   ├── settings.html         # Workspace settings
│   ├── team/
│   │   ├── team.html         # Team management page
│   │   └── invitations.html  # Pending invitations
│   └── partials/             # HTMX partials
│
└── [other apps remain unchanged]
```
...
def Dashboard(request):
    ...
    2. Has memberships → workspace_list (selector)
    3. No memberships → workspace_create (with prompt)
    """
    user = request.user
...
User logs in
    ↓
pages.Dashboard() ← Single smart router
    ├─ Has workspace? → workspace_dashboard (DONE - 1 redirect)
    ├─ Has memberships? → workspace_list (DONE - 1 redirect)
    └─ No memberships? → workspace_create (DONE - 1 redirect)
```
...
**Approach: Parallel Implementation → Gradual Switch**
...
- ✅ All workspace logic in one place (`apps/orgs/`)
- ✅ Clear URL patterns (`/workspace/`, `/workspace/<id>/team/`, etc.)
- ✅ Reusable components for common UI patterns
- ✅ Easy to find views and templates
- ✅ Permission checking is standardized

**User Experience:**
- ✅ Clear, consistent navigation
- ✅ Fewer redirects (1 instead of 3-4)
- ✅ Role badges show permissions clearly
- ✅ Features appear/disappear based on role automatically
- ✅ Empty states guide users to actions

**Documentation:**
- ✅ COMPLETE_USER_FLOW_GUIDE.md matches reality
...
company_dashboard → workspace_dashboard (add workspace_id parameter)
company_create → workspace_create
company_detail → workspace_detail
company_list → workspace_list
workspace_home → workspace_selector (more accurate name)
```

### Day 2: Consolidate Templates
```bash
# Move these templates:
templates/pages/company_dashboard.html → templates/workspace/dashboard.html
templates/company/company_list.html → templates/workspace/list.html
templates/company/company_detail.html → templates/workspace/detail.html
...
### Day 5: Test & Document
- Test critical flows (login → workspace → dashboard)
