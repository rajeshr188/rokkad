...
5. Sets: user.profile.workspace = workspace ✓
...
4. Sets: user.profile.workspace = public ✓
...
3. click "Switch" → switch_workspace(workspace_B.id)
4. Redirected to Workspace B / Dashboard
5. user.profile.workspace = Workspace B ✓
```
...
3. Workspace created with:
   ...
   - user.profile.workspace = new_workspace ✓
...
9. Clicks different workspace → workspace_select()
10. Redirected to: workspace_dashboard ✓
```
...
8. Next visit to /dashboard/:
   - Smart routing detects workspace selected ✓
...
2. ✅ Workspace exists (`get_object_or_404()`)
3. ✅ Workspace not deleted (`is_deleted=False`)
4. ✅ User has membership (`Membership.DoesNotExist`)
5. ✅ Permission denied if not member (`messages.error()`)
...
✅ **Multiple ways to switch**: Dropdown, list, management page  
✅ **Clear & reset options**: Return to public, select again  
✅ **Full audit trail**: All actions logged  
✅ **Security validation**: User membership verified  
✅ **User feedback**: Messages for all actions  
✅ **Mobile responsive**: Works on all devices  
✅ **Internationalized**: Multi-language support  
✅ **Smart routing**: Automatic dashboard direction  
