...
- PawnLoan disbursal, repayment, accrual, and capitalization corrections are explicit reversal source events linked one-to-one to immutable originals. Loans enforces administrator authority, mandatory reason, and newest-first dependency order. In `DEA`, only a `POSTED` original may be reversed and DEA compensates its voucher through the facade. In `DEFERRED`, only an intact `PENDING` original may receive a `PENDING` compensating source event, without fabricated voucher or journal identity. A source that already has a DEA effect cannot be domain-only reversed after the workspace changes to deferred. Cash-policy operational-only events receive compensating domain events without synthetic accounting vouchers.
...
- Release and renew prices the successor as a fresh contract under policy effective on the renewal date. Its advance interest and deducted fees remain distinct from source interest, source fees, principal paydown, and top-up even when the cashier sees one net handoff. Cash recognition credits advance interest to income; accrual recognition credits Unearned Revenue. The successor opening freezes per-item prepaid-interest amounts, and later accrual consumes that balance before creating any new customer charge. Operator confirmation is bound to a server-generated preview fingerprint and execution rejects changed economics.
