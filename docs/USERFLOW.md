# Rokkad — Complete User Flow

> **Architecture Note:** Rokkad uses `django-tenants` for multi-tenancy. There is a **public schema** (control plane — accounts, workspaces, billing, invitations) and a **tenant schema** per workspace (data plane — customers, loans, transactions). Every user navigates both planes during their lifecycle.
...
| Google OAuth | `/accounts/google/login/` — one-click sign-in via Google. Allauth creates the user on first login. |
...
2. `workspace_selector` view checks `profile.workspace`:
   - If a valid non-public workspace is already stored **and** the user still has membership → redirect straight to `workspace_dashboard`.
   - Otherwise → show the **Workspace Selector** page.
...
If the user clicks a workspace card → goes to `workspace_select`.
...
2. Call `_assert_workspace_access` — verifies user has an active `Membership` (or is a platform admin). If not → redirect to selector.
3. Call `profile.set_workspace(workspace)` — persists the chosen workspace FK on the user's `Profile`.
...
2. Choose "Switch Workspace" → `/orgs/workspace/` (selector, `?show_all=1`).
3. Click any workspace card → `workspace_select` sets new active workspace.
...
1. Navigate to **Company Settings → Team** (`/orgs/workspace/<id>/`).
2. Click "Invite Member" → `/orgs/team/invite/<workspace_id>/`.
...
- On acceptance: `control_plane.accept_invitation()` is called, which:
  ...
  2. Creates a `Membership` (user ↔ workspace ↔ role).
  3. Sets `profile.workspace` to the invited workspace.
- Redirect to `workspace_dashboard`.

**Path B — Invitee logs in and sees pending invitations:**
- URL: `/orgs/team/invitations/` (name: `team_invitations`).
- POST with `action=accept` and `invitation_id` → same `control_plane.accept_invitation()` path.
- POST with `action=decline` → invitation marked `DECLINED`.
...
- **Backend:** `_assert_workspace_access(required_permissions={...})` raises `PermissionDenied` (→ 403).
...
- `/subscriptions/` — dashboard showing current plan, expiry, usage.
- `/subscriptions/checkout/<plan_id>/` — subscribe to a plan.
- `/subscriptions/payment/` — payment details.
...
- **Member/Admin leaves:** `/orgs/workspace/<id>/leave/` — creates a self-removal; `profile.workspace` is cleared if it was this workspace.
- **Owner removes member:** Team management → Remove button → `team_remove_member` view.
  ...
  └─── /accounts/login/  ──────────────────────┤
       /accounts/google/login/                  │
                  ...
                  in profile already                multiple workspaces
                          ...
                          (Workspace Dashboard — role-gated UI)
                                        │
              ┌─────────────────────────┼────────────────────────┐
              │                         │                        │
