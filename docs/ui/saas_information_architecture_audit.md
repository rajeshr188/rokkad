...
- Phase 12.4 upgrades the customer/member portal shell in `docs/ui/customer_portal_shell_phase12.md`. `base_customer_portal.html` now has portal-only navigation and a `portal_content` block while keeping `/portal/...` routes absent. The tenant `/w/<workspace_slug>/...` alias map from the target route map is verified complete except for the separate portal routes and intentionally skipped Contact.
