...
- Phase 13.3 has started direct-render slug entry wrappers. `/w/<workspace_slug>/parties/` now renders the existing Party list view in place and keeps Party authorization as the source of truth. `/w/<workspace_slug>/inventory/` now renders the existing Product home view in place while detailed Product routes keep their existing action guards. Loans, accounting, and DEA-backed entries still redirect until each target view is verified.
