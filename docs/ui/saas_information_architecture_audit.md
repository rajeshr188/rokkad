...
- Phase 11 final review in `docs/ui/workspace_slug_phase11_review.md` closes the workspace slug route-map rollout for tenant ERP and workspace settings. The remaining target route-map gap was the separate customer/member portal.
- Phase 12.1 starts customer/member portal IA in `docs/ui/customer_portal_phase12_plan.md`. Its original route-absent checkpoint is now superseded by the Party read-only portal rollout.
- Phase 12.2 chooses a Party-backed portal identity model in `docs/ui/customer_portal_identity_phase12.md`. The later Party rollout implements the explicit tenant `PartyPortalAccess` binding and makes `resolve_portal_identity()` database-backed.
- Phase 12.3 defines customer/member portal selector contracts in `docs/ui/customer_portal_selector_contracts_phase12.md`. The later Party rollout replaces the fail-closed selector stubs with Party-scoped dashboard, loan, invoice, payment, document, and statement summaries.
- Phase 12.4 upgrades the customer/member portal shell in `docs/ui/customer_portal_shell_phase12.md`. The shell now has enabled tenant portal navigation for read-only customer routes and still avoids ERP/sidebar leakage.
- Phase 12 closeout in `docs/ui/customer_portal_phase12_review.md` is superseded by the Party read-only portal MVP: tenant `/portal/...` routes are live, public `/portal/...` remains absent, and routes require active `PartyPortalAccess`.
...
- Phase 14.2 completes the low-risk compressed batch. Visible Party GET links prefer slug aliases when workspace context exists; Product/Inventory, Rates, Notify, and Data Tools export have read-only slug aliases. Mutation routes, nested Party forms, Party success redirects, Girvi, DEA, deeper UI redesign, and legacy-root cleanup remain deferred.
- Phase 15 completes the compressed Girvi/Loans read-only alias batch in `docs/ui/tenant_girvi_route_canonicalization_phase15.md`. Loan list/detail/tab/PDF/report aliases are live under `/w/<workspace_slug>/loans/...`; Girvi lifecycle mutations, repayment, release, custody, operations-console retry, DEA, deeper UI redesign, and legacy-root cleanup remain deferred.
- Phase 16 completes the compressed DEA/Accounting read-only alias batch in `docs/ui/tenant_dea_route_canonicalization_phase16.md`. Top-level DEA section aliases direct-render existing views, and read-only accounting/commodity aliases are live. Posting, business-event confirm, period mutation, reconciliation import/match, opening balance, commodity setup, deeper UI redesign, and legacy-root cleanup remain deferred.
- Phase 17 closes the compressed route-canonicalization work in `docs/ui/tenant_legacy_root_compatibility_phase17.md`. Legacy tenant roots remain active compatibility routes; `/w/<workspace_slug>/...` is the preferred read-only/navigation surface where aliases exist. Remaining route work is module-specific POST/HTMX/success-redirect migration, not a blanket legacy-root redirect.
