...
Iâ€™ll inspect the product app models and related architecture files first, then provide a focused code review with concrete enhancement recommendations prioritized by impact.
...
I found the product app under apps/tenant_apps/product; next Iâ€™m reading its model modules and related stock/domain models to evaluate architecture quality end-to-end.
...
Iâ€™ve finished the core model files and found several architecture smells already; Iâ€™m now checking managers, attribute plumbing, and alternative model implementations to separate active design from legacy/experimental code.
...
Iâ€™ve confirmed the app has very few tests and several legacy indicators; next Iâ€™m reading the key forms/views that exercise these models so the architecture review reflects real usage, not just schema shape.
...
## Catalog Domain â€” Architecture Review

### Domain inventory

| Model | Role | Active |
|---|---|---|
| `Category` | MPTT hierarchy (gold/silver/etc.) | âœ… |
| `ProductType` | Template for products (ring/chain/etc.) | âœ… |
| `Attribute` + `AttributeValue` | Shared key-value taxonomy | âœ… |
| `Product` | Concrete item per type+attribute combination | âœ… |
| `ProductVariant` | Sellable unit (SKU) of a Product | âœ… |
| `ProductImage` + `VariantImage` | Photo catalogue | âœ… |
| All of new_product.py | Alternative, richer model design | âŒ never imported anywhere |

---

### Bugs (broken behavior in production)

**1. `MPPTMeta` typo â€” MPTT ordering is silently ignored**

product.py (also price.py):
```python
class MPPTMeta:                  # wrong â€” django-mptt looks for MPTTMeta
...
**7. Category delete is GET-based â€” CSRF-unsafe destructive action**
...
Two product types named "Ring" can be created. Since `Product.name` is generated from `product_type.name + attributes`, duplicate product type names produce product name collisions that are caught only by the `unique=True` constraint on Product â€” at save time with a cryptic IntegrityError.
...
#### E. `ProductVariantForm` exposes only `name` â€” SKU and code are uneditable via UI
...
**Phase 1 â€” Fix bugs that corrupt data or break UI**
...
4. Fix `MPPTMeta` â†’ `MPTTMeta` on `Category` and `PricingTier`
5. Fix `ProductVariant.get_attributes()` to use variant-level attributes

**Phase 2 â€” Performance and consistency**
...
**Phase 3 â€” Attribute architecture decision** (requires migration)
- Either consolidate HStore â†’ JSONField, add GIN index, enforce referential integrity via a signal/constraint
...
### The current design â€” one blob per object

```
Product
  attributes  = HStoreField  â†’  {"14": "37", "15": "42"}   â† Attribute PK â†’ AttributeValue PK
  jattributes = JSONField     â†’  {"Metal": "Gold", "Colour": "Yellow"}
```
...
### What new_product.py does instead â€” four dedicated relational tables

```
ProductType â”€â”€â”€ AttributeProduct  â”€â”€â”€ Attribute â”€â”€â”€ AttributeValue
                     â”‚                                    â”‚
                (sort_order)                         (sort_order)
                     â”‚
               AssignedProductAttribute  â†â”€â”€ Product
                     â”‚
                   values (M2M) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ AttributeValue
```

The four key models:

**`AttributeProduct`** â€” the junction between `ProductType` and `Attribute`:
```python
...
**`AssignedProductAttribute`** â€” the junction between `Product` and its chosen values:
```python
...
#### 1. Referential integrity â€” values can't go stale

Current design:
```python
# In Product.attributes (HStore): {"14": "37"}
# If AttributeValue pk=37 is deleted â†’ orphan PK, no error, silently wrong
...
# One assignment can hold {"Gold", "Rose Gold"} â€” multi-select attributes work naturally
```
...
Current design â€” to find "all Gold rings":
```python
...
**Step 1 (now)** â€” Drop `HStoreField`, keep only `jattributes` (JSONField). Removes one broken copy, simplifies the form save, requires one migration + deleting `attributes =` field and its references.

**Step 2 (later, when ready)** â€” Adopt normalized tables from new_product.py with a data migration from `jattributes`. At that point `jattributes` becomes the intermediary that feeds the migration script, then gets dropped too.

The JSONField consolidation is low-risk and gives you immediate benefit (less sync surface, one source of truth). The normalized design is the long-term end state and new_product.py already has it written â€” it just needs to be hooked in.

Want me to implement Step 1 â€” remove `HStoreField.attributes` from both `Product` and `ProductVariant` and consolidate everything onto `jattributes`?
