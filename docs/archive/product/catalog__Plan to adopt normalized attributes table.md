...
**TL;DR** â€” Replace `jattributes` JSONField with four normalized DB tables from new_product.py. `jattributes` stays alive as the data bridge for the migration script, then gets dropped. Four phases, app keeps working throughout.
...
|---|---|---|
| Referential integrity | None â€” rename `AttributeValue` and jattributes silently breaks forever | FK cascade â€” rename/delete propagates automatically |
...
| Filter by attribute | `jattributes__contains` GIN â€” fragile, no multi-value | `JOIN: attributes__values__slug="gold-22k"` |
...
### Phase 1 â€” Add normalized tables alongside jattributes (schema only)
*jattributes untouched â€” app keeps working*
...
3. Change `ProductType.product_attributes` M2M â†’ `through=AttributeProduct`. Same related name, so `product_type.product_attributes.all()` still works everywhere.
4. Change `ProductType.variant_attributes` M2M â†’ `through=AttributeVariant`.
...
### Phase 2 â€” Data migration (jattributes â†’ normalized)
*Depends on Phase 1*

10. Write management command `migrate_jattributes_to_normalized` â€” reads each `Product.jattributes` dict, resolves `Attribute` by name, `AttributeValue` by name, creates `AttributeProduct` + `AssignedProductAttribute` + value links. Same for `ProductVariant`. Logs skips.
11. Run on a single test tenant first, verify row counts match.

### Phase 3 â€” Switch application code to read/write normalized tables
...
15. Templates: minimal changes â€” `get_attributes()` already abstracts the source.

### Phase 4 â€” Drop jattributes
...
**Relevant files**
- models/product.py â€” 4 new models, 2 M2M `through=` changes, fields on `Attribute`/`AttributeValue`
- attributes.py â€” 3 helpers rewritten in Phase 3
- forms.py â€” `AttributesMixin` rewritten in Phase 3
- `apps/tenant_apps/product/management/commands/migrate_jattributes_to_normalized.py` â€” new in Phase 2
...
3. Phase 3: create product with attributes â†’ appears on detail page; edit â†’ updates correctly
4. Phase 4: `manage.py check` clean; no `jattributes` column; all attribute flows work

**Explicit exclusions**
- Not adopting `PublishableModel`, `SeoModel`, `TranslationProxy`, `MoneyField` from new_product.py â€” separate concerns
- Not implementing `sort_by_attribute()` queryset in Phase 1 â€” add after full cutover
- new_product.py stays as reference, not imported
- `SortableModel` mixin not pulled in (needs uninstalled packages) â€” use plain `PositiveIntegerField(null=True)` for sort_order
