...
# Girvi â†” DEA Payment Integration â€” Detailed PR Plan

**Date**: March 23, 2026  
**Branch base**: `dea-kiss`  
**Author**: rajeshr188  
**Status**: ðŸš§ IN PROGRESS (PR-1/PR-2/PR-3 implemented, PR-4 underway)  
...
The girvi payment form at `/girvi/loanpayment/create/<pk>/` still writes to the legacy `LoanPayment` model. `LoanPayment.save()` attempts to post via `BusinessDoc._auto_post_to_accounting()` using voucher type `"LOAN_REPAY"` â€” but `LoanRepaymentRule` is **not decorated** with `@register_rule`, causing a silent `RuleNotFoundError` on every save. Every girvi payment made through the current UI produces **zero accounting entries**.
...
- `Release` events (collateral return) have zero DEA integration â€” `Release` does not extend `BusinessDoc`.
...
2. Calls `loan.create_payment(amount, date, method, ref, interest, principal, description, is_final, create_release, user)` â€” creates `PaymentVoucher` with `auto_post=False`.
3. Immediately calls `create_and_post_voucher_for_doc(doc=payment, voucher_type=payment.get_voucher_type())` â€” posts to ledger via the correct registered rule.
...
|---|---|---|---|
| G-1 | Girvi repayment UI writes `LoanPayment`, not `PaymentVoucher` | ðŸ”´ Critical | PR-1 |
| G-2 | `LoanRepaymentRule` unregistered â†’ silent posting failure | ðŸ”´ Critical | PR-1 (remove; rule stays for legacy) |
| G-3 | Loan disbursal has no `PaymentVoucher` â†’ LOAN_RECEIVABLE never opened | ðŸ”´ Critical | PR-2 |
| G-4 | `TakenLoan` has no repayment UI | ðŸŸ  High | PR-3 |
| G-5 | `Release` has zero DEA integration | ðŸŸ  High | PR-4 |
| G-6 | `TakenLoan` disbursal (we receive money from lender) has no `PaymentVoucher` | ðŸŸ  High | PR-2 |
| G-7 | Loan detail "Payments" tab shows `LoanPayment` records, not `PaymentVoucher` | ðŸŸ¡ Medium | PR-1 |
| G-8 | `LoanPaymentCreateForm` and girvi payment URLs are a dead end | ðŸŸ¡ Medium | PR-5 |
| G-9 | `get_voucher_type()` returns `PAYMENT_OTHER` when source_content_type is null | ðŸŸ¡ Medium | PR-1 (guard) |
...
```
PR-1 â”€â”€â–º PR-2 â”€â”€â–º PR-3
              â”‚
              â””â”€â”€â–º PR-4 â”€â”€â–º PR-5
```

### PR-1: Wire Girvi Repayment UI â†’ PaymentVoucher
...
## 6. PR-1 â€” Wire Girvi Repayment UI â†’ PaymentVoucher
...
if form.is_valid():
    ...
    payment.loan = loan
    payment.save()   # â† LoanPayment.save() â†’ LOAN_REPAY â†’ RuleNotFoundError (silent)
```
...
**File**: `apps/tenant_apps/girvi/views/loan.py` â€” `loan_detail_payments_tab` view
...
Add "New Payment" button â†’ `{% url 'girvi_loanpayment_create' loan.pk %}`.
...
def get_voucher_type(self) -> str:
    if not self.source_content_type_id:
        raise ValueError("PaymentVoucher.get_voucher_type(): source_content_type is null â€” cannot determine voucher type.")
    ...
```
...
- [ ] Create GivenLoan payment through `/girvi/loanpayment/create/<pk>/` â†’ `PaymentVoucher` created, `posted=True`
- [ ] Open DEA payment list â†’ payment appears
- [ ] Open girvi loan detail payments tab â†’ `PaymentVoucher` records shown with Posted badge
...
## 7. PR-2 â€” Loan Disbursal PaymentVoucher
...
**File**: `apps/tenant_apps/girvi/views/loan.py` â€” `loan_transition_view`
...
|---|---|---|---|---|---|
| Status â†’ DISBURSED | GivenLoan | PAYMENT | DISBURSAL | `GIVENLOAN_PAYMENT` | Dr Loan_Receivable, Cr Cash |
| Status â†’ DISBURSED | TakenLoan | RECEIPT | DISBURSAL | `TAKENLOAN_RECEIPT` | Dr Cash, Cr Loan_Payable |
...
Confirm `@register_rule("GIVENLOAN_PAYMENT")` (or `"LOAN_DISBURSE"` â€” verify current key). If key mismatches `payment.get_voucher_type()`, alias-register.

### Verification

- [ ] Create `GivenLoan` â†’ set status DISBURSED â†’ `PaymentVoucher(direction=PAYMENT, type=DISBURSAL)` created
- [ ] Journal entry: Dr Loan_Receivable, Cr Cash for correct amount
- [ ] Transition again â†’ idempotent, no duplicate entry
- [ ] Create `TakenLoan` â†’ DISBURSED â†’ `PaymentVoucher(direction=RECEIPT, type=DISBURSAL)` created
- [ ] Journal entry: Dr Cash, Cr Loan_Payable

---

## 8. PR-3 â€” TakenLoan Repayment UI
    ...
    if request.method == "POST" and form.is_valid():
        ...
        try:
            create_and_post_voucher_for_doc(
                doc=payment,
                user=request.user,
                voucher_type_input=payment.get_voucher_type(),  # â†’ "TAKENLOAN_PAYMENT"
            )
            payment.posted = True
            payment.save(update_fields=["posted"])
...
TakenLoan Repayment PaymentVoucher posted:
  Dr Loan_Payable (lender subledger)   â† reduce liability
  Cr Cash                               â† cash outflow
```

### Verification

- [ ] Open TakenLoan detail â†’ "Record Repayment" button visible
- [ ] Submit form â†’ `PaymentVoucher(direction=PAYMENT, source=takenloan)` created and posted
- [ ] `TAKENLOAN_PAYMENT` rule fired: Dr Loan_Payable, Cr Cash
- [ ] `loan.total_paid` increases; `loan.outstanding_balance` decreases

---

## 9. PR-4 â€” Release Accounting Integration
...
def record_loan_release(release, created_by):
    ...
    payment = PaymentVoucher.objects.create(
        ...
        payment_method=PaymentMethod.CASH,
        description=f"Release: {loan.loan_id} â€” {release.release_id}",
...
- [ ] Create Release for a loan with zero outstanding â†’ `PaymentVoucher` with `total_amount=0` created and posted
- [ ] Create Release for loan with unpaid balance â†’ write-off entry created (Dr Loan_Receivable, Cr Interest_Income)
...
## 10. PR-5 â€” Deprecate Old `LoanPayment` Model
...
| D-2 | `record_loan_disbursal()` triggered only on status transition DISBURSED (not at loan creation) | Loan creation â‰  disbursal; loans can be in Created/Approved state without payout | Auto-post on `GivenLoan.save()` would fire on every update |
...
PR-1: Wire Girvi Repayment UI â†’ PaymentVoucher
  ...
  [ ] Manual flow tested: create payment â†’ DEA journal entries visible
  [ ] PR link:
  [ ] Merge date:
...
PR-2: Loan Disbursal PaymentVoucher
    ...
    [x] TakenLoan DISBURSED â†’ TAKENLOAN_RECEIPT verified
    [x] manage.py check: 0 issues
  [ ] Manual flow tested: status â†’ DISBURSED â†’ PaymentVoucher in DEA
  [ ] PR link:
  [ ] Merge date:
...
PR-4: Release Accounting Integration
  [ ] Branch created: feature/girvi-dea-pr4-release-accounting
    ...
    [x] Manual flow validated (synthetic): LOAN=Z00004 â‚¹1000 â†’ GIVENLOAN_RELEASE voucher POSTED â†’ JE created
        LTXN: DR:Interest Paid | CR:Loans & Advances | AMT:â‚¹1,000.00
  [ ] PR link:
  [ ] Merge date:
...
PR-5: Deprecate Old LoanPayment Model
  [ ] Branch created: feature/girvi-dea-pr5-deprecate-loanpayment
    ...
    [x] Migration 0013: archive legacy LoanPayment rows â†’ draft PaymentVoucher
      ...
      - Result on jcl schema: 3 legacy LoanPayments â†’ 3 draft PaymentVouchers
    [x] manage.py check: 0 issues
