...
<tr>
    ...
    <td>{{ loan.gold_weight|floatformat:3 }}g @ â‚¹{{ loan.gold_value|floatformat:0 }}</td>
    <td>â‚¹{{ loan.total_due|floatformat:0 }}</td>
...
<div class="card">
    ...
    <p>Total Due: â‚¹{{ non_perf.total_due|floatformat:0 }}</p>
    <p>Shortfall: â‚¹{{ non_perf.total_due|add:non_perf.total_collateral_value|floatformat:0 }}</p>
    
    <h6>Gold: {{ non_perf.metals.Gold.pure_weight|floatformat:2 }}g @ â‚¹{{ non_perf.metals.Gold.value|floatformat:0 }}</h6>
    <h6>Silver: {{ non_perf.metals.Silver.pure_weight|floatformat:2 }}g @ â‚¹{{ non_perf.metals.Silver.value|floatformat:0 }}</h6>
</div>
```
...
with_duration_metrics()
    â†“
    â””â”€â†’ with_interest_metrics() needs this

with_metal_weights()
    â†“
    â””â”€â†’ with_current_value() needs this
    â””â”€â†’ with_overdue_status() needs this (if comparing value < due)
...
**In your IDE:**
Add as a snippet in VS Code: Settings â†’ User Snippets â†’ python
