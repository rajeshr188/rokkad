...
âœ… **Clear Custody Status**: Every item has IN_VAULT, WITH_LENDER, or WITH_CUSTOMER  
âœ… **Automatic Return Workflow**: When customer wants to release, system returns items from lenders first  
âœ… **Multi-Item Bundling**: Bundle items from different customers as collateral for one TakenLoan  
âœ… **Full History**: Complete audit trail of all repledge/return events  
âœ… **Validation**: Cannot release items that are with lenders (without return first)  
âœ… **LTV Tracking**: Loan-to-value ratios for risk management  
...
1. User clicks "Release Loan"
   â†’ Redirects to custody check page

2. System shows:
   - 3 items in vault âœ“
   - 2 items with Lender A âš ï¸
...
3. User clicks "Release with Auto-Return"
   ...
   with transaction.atomic():
       ...
       for item in all_items:
           ...
           # custody_status: IN_VAULT â†’ WITH_CUSTOMER
...
1. Navigate to /girvi/repledge/create/
   â†’ Shows all available items grouped by customer

2. Select items:
   - Customer A: 2 gold rings (â‚¹50,000 total)
   - Customer C: 1 gold chain (â‚¹30,000 total)
   - Total collateral: â‚¹80,000

3. Fill form:
   - Lender: Lender B
   - Loan Amount: â‚¹60,000 (75% LTV âœ“)
   - Interest Rate: 1.5%
   - Date: Today
...
2. Click "View Collateral" â†’ Shows:
   - Customer A: 2 items
   - Customer C: 1 item
   - Total collateral value: â‚¹80,000
   - LTV ratio: 75%
...
- âŒ Already repledged (repledged_to is set)
- âŒ Not in vault (custody_status != IN_VAULT)
- âŒ Loan is released

### Cannot Release Item If:
- âŒ With lender (custody_status == WITH_LENDER)
  - **Solution**: Use `release_with_return_workflow()`
- âŒ Already with customer (custody_status == WITH_CUSTOMER)

### Cannot Close TakenLoan If:
- âŒ Collateral items not returned
...
âœ… **Physical custody tracking** - Always know where items are  
âœ… **Automatic workflows** - Return from lenders when releasing  
âœ… **Multi-customer bundling** - Use items from different customers as collateral  
âœ… **Full audit trail** - Complete history of all movements  
âœ… **Validation** - Prevents invalid operations  
âœ… **Backward compatible** - Old RepledgedLoanItem data migrated  

Your requirements are fully met:
- âœ… Items get repledged very often â†’ Efficient tracking
- âœ… Customers can release while repledged â†’ Auto-return workflow
- âœ… Full history preferred â†’ RepledgeHistory model
- âœ… No multiple repledges without release â†’ Validation enforced
- âœ… Bundle items from different customers â†’ Multi-item collateral
