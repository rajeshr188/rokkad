...
Need loan data?
â”‚
â”œâ”€ For table display (list all loans with metrics)?
â”‚  â””â”€â–º Use: Loan.objects.unreleased().for_table_display()
â”‚       Returns: All annotations (weights, amounts, interest, values, status)
â”‚
â”œâ”€ For dashboard (aggregate stats)?
â”‚  â””â”€â–º Use: Loan.objects.for_dashboard_metrics()
â”‚       Returns: Same annotations, ready to aggregate
â”‚
â”œâ”€ Need custom filter (e.g., only overdue)?
â”‚  â””â”€ Filter condition?
â”‚     â”œâ”€ is_overdue = True?
â”‚     â”‚  â””â”€â–º Use: Loan.objects.overdue()
â”‚     â”‚
â”‚     â”œâ”€ months_since_created > 12?
â”‚     â”‚  â””â”€â–º Use: Loan.objects.long_dead(months=12)
â”‚     â”‚
â”‚     â”œâ”€ status = 'good_standing'?
â”‚     â”‚  â””â”€â–º Use: Loan.objects.good_standing()
â”‚     â”‚
â”‚     â””â”€ Custom condition?
â”‚        â””â”€â–º Use: Loan.objects.for_table_display().filter(your_condition...)
â”‚
â”œâ”€ Just need specific annotations?
â”‚  â””â”€ Which ones?
â”‚     â”œâ”€ Only duration? â†’ with_duration_metrics()
â”‚     â”œâ”€ Only interest? â†’ with_interest_metrics()
â”‚     â”œâ”€ Only weights? â†’ with_metal_weights()
â”‚     â”œâ”€ Only amounts? â†’ with_itemwise_amounts()
â”‚     â”œâ”€ Only values? â†’ with_current_value()
â”‚     â”œâ”€ Only status? â†’ with_overdue_status()
â”‚     â””â”€ Multiple? â†’ Chain them: .with_duration_metrics().with_interest_metrics()
â”‚
â””â”€ Using dashboard service directly?
   â””â”€ Use: Loan.objects.non_performing_loans_stats()
...
**When to use:**
- âœ… Table views with multiple metrics
- âœ… Reports showing loan details
- âŒ NOT for aggregations (use `for_dashboard_metrics()`)
- âŒ NOT if you only need 1-2 fields (use individual methods)
...
- âœ… Report totals
- âŒ NOT for displaying individual rows
...
**Pick your method:**
- Display all metrics â†’ `for_table_display()`
- Aggregate stats â†’ `for_dashboard_metrics()`
- Just filter â†’ `overdue()`, `good_standing()`, `long_dead()`
- Just one metric â†’ Individual methods like `with_metal_weights()`
- Dashboard analysis â†’ `non_performing_loans_stats()`, `long_dead_loans_stats()`
