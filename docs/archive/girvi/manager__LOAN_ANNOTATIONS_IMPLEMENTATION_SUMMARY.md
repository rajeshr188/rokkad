...
| `pure_gold_weight` | gold_weight Ã— purity% | Actual gold content |
| `gold_loanamount` | Sum of gold item loan amounts | Borrowed against gold |
| `gold_value` | pure_gold_weight Ã— current_gold_rate | Current market value |
| `total_interest` | interest Ã— months_since | Accrued interest |
...
### Step 1: Code Updates (âœ… Already Done)
...
print(f"{stats['count']} overdue, shortfall: â‚¹{stats['shortfall']}")
```
