...
1. Build LoanTemplate list/create/update/delete screens in app UI. âœ…
2. Add template activation and default actions. âœ…
3. Restrict access to tenant owner/admin roles. âœ…

### Phase B: Tenant-facing frame management

Status: Implemented for baseline CRUD + starter seeding.

1. Build frame list and editor for each template. âœ…
2. Support frame_name, template_type, field_type, x/y/width/height, font settings. âœ…
3. Preserve current uniqueness constraints and model validation. âœ…
4. Add starter-frame seeding action for first-time setup. âœ…

### Phase C: Preview and coordinate correctness

Status: Implemented.

1. Add in-app preview page equivalent to admin preview. âœ…
2. Address coordinate-system mismatch explicitly. âœ…
3. PDF uses bottom-left origin. âœ…
4. Browser preview uses top-left origin. âœ…
5. Add conversion guidance and helper overlays. âœ…

### Phase C.1: Onboarding and usability hardening

Status: Implemented.

1. Surface starter pack download in list, create, and detail workflows. âœ…
2. Add explicit mapping guidance for uploaded PDF assets. âœ…
3. Add paper-size references for page dimension input. âœ…
4. Add starter frame generation for easier first-time frame placement. âœ…
...
1. Frame bounds validation exists at model level. âœ…
2. Duplicate/clone template action. â³
3. Lightweight versioning/audit log for template edits. â³
4. Strong publish/readiness checks before default activation. â³
