...
**Deliverables:**
- âœ… `_assert_workspace_access()` helper implemented (9 endpoints integrated)
- âœ… Helper unit tests: 6/6 passing (denials, platform-admin bypass, permission checks)
- âœ… Endpoint-level tests: workspace_select (2 tests, deny redirect + allow success)
...
**Deliverables:**
- âœ… Template audit completed (all 5 key navigation templates checked)
- âœ… Canonical routes verified in use: workspace_selector, workspace_dashboard, workspace_detail, workspace_preferences, team_invitations, team_invite
- âœ… Deprecated route names verified absent: orgs_invite_delete, company_invite_list, team_invite_delete
- âœ… OrgNavigationFlowTests class: 5/5 tests passing

**Implementation details:**
- Test class: `OrgNavigationFlowTests` in [apps/orgs/tests.py](../../apps/orgs/tests.py)
- Tests implemented:
  1. `test_sidebar_uses_canonical_workspace_routes` â€” Verifies sidebar.html uses canonical routes
  2. `test_workspace_dashboard_uses_canonical_routes` â€” Verifies workspace_dashboard.html uses canonical routes
  3. `test_profile_page_uses_team_invitations_route` â€” Verifies profile.html links to team_invitations
  4. `test_main_navigation_uses_canonical_routes` â€” Verifies tenant.html uses canonical routes
  5. `test_no_deprecated_route_names_in_templates` â€” Verifies absence of deprecated route names
...
**Deliverables:**
- âœ… Domain/path mismatch guard confirmed in middleware
- âœ… Platform-admin bypass verified and tested
- âœ… DomainPathMismatchTests class: 5/5 tests passing
...
**Deliverables:**
- âœ… Added explicit invitation status model with canonical states: `pending`, `accepted`, `declined`, `revoked` (+ derived `expired` at runtime)
- âœ… Added deterministic transition helpers in model: `mark_declined`, `mark_revoked`, state-aware `accept`
- âœ… Added migration `0022_companyinvitation_responded_at_and_more` with backfill for already accepted invitations
- âœ… Updated invitation flows in views to be state-aware and idempotent
- âœ… Updated invitation templates to render status from lifecycle state
- âœ… InvitationLifecycleStateTests class: 6/6 tests passing
...
**Deliverables:**
- âœ… Last-owner guardrail added for member removal
- âœ… Last-owner guardrail added for role-demotion flow
- âœ… Self-leave policy implemented:
  - non-owner self-leave allowed
  - owner self-leave blocked until ownership transfer
- âœ… Removed member profile workspace is cleared immediately when it points to removed workspace
- âœ… MembershipLifecycleGuardrailTests class: 5/5 tests passing
...
**Deliverables:**
- âœ… Added dedicated control-plane service module: `apps/orgs/services/control_plane.py`
- âœ… Centralized Company/Membership/Invitation write operations into service functions
- âœ… Enforced public schema context for control-plane writes via shared schema wrapper
- âœ… Added audit events for all mutation operations handled by control-plane services
- âœ… Refactored org views to use service-layer mutations instead of direct model writes
- âœ… ControlPlaneIntegrityTests class: 3/3 tests passing
...
1. âœ… `manage.py check` â†’ `System check identified no issues (0 silenced)`
2. âœ… `manage.py test apps.orgs.tests` â†’ `Ran 63 tests ... OK`
3. âœ… Focused middleware/security coverage confirmed in passing suite:
