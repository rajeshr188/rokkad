...
> **Updated 2026-05-03** â€” synchronous facade pattern adopted (outbox deferred).
> All cross-app DEA imports must go through `dea/facade.py` only.

1. ~~Remove direct `girvi <-> dea` model imports~~ âœ… **DONE** â€” `girvi/service_modules/payment.py` uses `dea.facade` only.
2. ~~Move `contact -> dea` signal~~ âœ… **DONE** â€” `contact/signals.py` uses `dea.facade.ensure_customer_account`.
3. ~~Remove direct `sales -> dea` model imports~~ âœ… **DONE** â€” `sales/models/sale.py` and `receipt.py` use `dea.facade` only.
4. ~~Remove direct `purchase -> dea` model imports~~ âœ… **DONE** â€” `purchase/models/purchase.py` and `payment.py` use `dea.facade` only.
5. ~~Fix `dea/models/payment.py` reverse import of `GivenLoan`/`TakenLoan`~~ âœ… **DONE** â€” `source_loan` uses existing `source_document` GenericForeignKey.
6. ~~Add CI import guardrail~~ âœ… **DONE** â€” `scripts/check_dea_boundary.py` (exits 1 on violations, run with `--list`).
7. Remaining 25 violations to fix in subsequent PRs *(tracked by guardrail)*:
   - `girvi/models/loan.py`, `loan_refactored.py` â€” `BusinessDoc` inheritance + inline PaymentVoucher imports
   - `girvi/service_modules/accrual.py` â€” interest accrual posting (needs facade extension)
   - `girvi/views/loanpayment.py` â€” view calls posting engine directly
   - `girvi/views/dashboard.py`, `girvi/selectors.py` â€” read-only PaymentVoucher queries
   - `approval/models/` â€” JournalEntry GenericRelation (â†’ string reference)
   - `product/models/stock.py` â€” JournalEntry GenericRelation (â†’ string reference)
   - `pages/views.py` â€” dashboard aggregation queries
   - `orgs/.../seed_tenant_defaults.py` â€” admin seeding (allowlist acceptable)
   - Test files â€” direct posting rule imports (test-only, low risk)
