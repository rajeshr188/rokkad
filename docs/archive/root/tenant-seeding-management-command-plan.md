...
**TL;DR**: Move 11+ non-idempotent data seeding migrations â†’ idempotent management commands + post-migrate signals + YAML fixtures. Enables clean clone-schema (DDL-only migrations) + global-updatable defaults. **Two phases: Build infrastructure, then refactor existing migrations.**
...
2. **Create management command** `seed_tenant_defaults`
   ...
   - Loads all fixtures in dependency order (roles â†’ chart â†’ products â†’ notices â†’ rates)
...
- `orgs/0022_role_constraints.py` â€” Add UniqueConstraint on roles
- `product/0023_product_indexes.py` â€” Add performance indexes
- `dea/0015_dea_schema_only.py` â€” DDL equivalent of old 0002 (no data)
- `notify/0004_noticeconfig_indexes.py` â€” Indexes on notice configs

### **Phase 3: Integration & Validation**

1. **Update tenant creation flow** â€” Trigger `seed_tenant_defaults` on `Company.objects.create()`
2. **Create test suite** â€” `tests/test_seeding.py` with idempotency + data integrity tests
3. **Validate migrations** â€” Run clean migrate, verify seeding automatic
4. **Backfill existing tenants** â€” Run `seed_all_tenants --force`

### **Phase 4: Cleanup & Documentation**

1. **Remove deleted migrations** from git
2. **Create `TENANT_DATA_SEEDING.md`** â€” Architecture docs for team
3. **Create `FIXTURE_MAINTENANCE.md`** â€” How to update fixtures
...
- **Replace**: `product/migrations/0003_auto_fixture.py` â†’ new DDL-only version
- **Delete**: `dea/migrations/0002_initial_fixture.py`
- **Delete/Stub**: `notify/migrations/0003_seed_default_notice_types.py`
- signals.py â€” Add trigger for `seed_tenant_defaults`
- base.py â€” Ensure FIXTURE_DIRS configured
...
1. âœ… `python manage.py seed_tenant_defaults --dry-run` â†’ Lists actions
2. âœ… `python manage.py test tests/test_seeding.py` â†’ All pass
3. âœ… Verify: `Role.objects.count() == 4`, `Ledger.objects.count() == 70`

**Phase 2 (Refactoring)**:
1. âœ… Drop DB, run `migrate` â†’ DDL only, no data duplication
2. âœ… Create test tenant â†’ Auto-seeded via signal
3. âœ… `seed_all_tenants --force` â†’ Idempotent (counts unchanged)

**Phase 3 (Integration)**:
1. âœ… New tenant created â†’ Defaults auto-seeded
2. âœ… Clone schema test â†’ Both schemas identical post-seed

---

## Decisions & Key Assumptions

- **Clone schema approach**: DDL-only (structure copied, data seeded fresh) â†’ allows shared defaults
...
- **Post-migrate signal**: Auto-seeds on first migrate (idempotent â†’ safe to run multiple times)

---

## Further Considerations

1. **Natural Keys for Fixtures** â†’ Needed for loaddata idempotency
   - Action: Verify/add `natural_key()` methods to Ledger, NoticeTypeConfig, etc. before Phase 1

2. **MPPT Tree Rebuild** â†’ Account chart uses nested sets
   - Action: Call `Ledger.objects.rebuild()` after loading `account_chart.yaml`

3. **Backward Compatibility** â†’ Removing migrations from git won't break production
...
4. **Permissions Model** â†’ Clarify if Role just stores name or also permissions
