...
# Contact App â€” Django 6 Native Partials + HTMX Architecture

## Templates
- templates/contact/customer_detail_improved.html â€” main detail page
  ...
  - section-level partials (full section container replacement â€” main CRUD target):
    - customer-contacts-section
    - customer-addresses-section
    - customer-proofs-section
    - customer-relationships-section
  - page partial: customer-detail inline (HTMX navigation target when HX-Target is the top container)  - profile header partial: `customer-hero` â€” wraps the avatar/name/edit-button hero strip; updated in-place after customer edit without a full page redirect  - inline JS listens for contactModalClose event and closes the modal after a successful POST

- templates/contact/customer_list_improved.html â€” list page
  - customer-filter-panel â€” filter sidebar/inline panel
  - customer-list-results â€” paginated card/row results (HTMX filter + pagination target)
  - customer-content inline â€” full page content (used when HX-Target is content, i.e. full HTMX page nav)

## View layer â€” common helpers (apps/tenant_apps/contact/views/common.py)
- CUSTOMER_DETAIL_TEMPLATE = "contact/customer_detail_improved.html"
- get_customer_for_detail(pk) â€” loads Customer with full prefetch: address, contactno, proofs, pics, relationships_created__related_customer
- render_customer_detail_fragment(request, customer, fragment_name, context=None, status=200, headers=None) â€” renders CUSTOMER_DETAIL_TEMPLATE#<fragment_name> with optional extra headers
...
- DELETE / set-default: call render_customer_detail_fragment(...) with just the section name (no extra headers needed â€” no modal to close).
...
- _get_customer_list_context(request) â€” builds CustomerFilter + Paginator(12) context.
- customer_list():
...
- customer_detail():
  ...
  - POST update success: returns `customer-hero` fragment with `HX-Retarget: #customer-hero`, `HX-Reswap: outerHTML`, `HX-Trigger: contactModalClose` â€” refreshes hero in-place and closes modal.
## Key conventions
- No {% load partials %} â€” Django 6 native partials require no tag library.
