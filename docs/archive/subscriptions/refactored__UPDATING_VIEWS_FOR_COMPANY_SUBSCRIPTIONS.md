...
class InvoiceDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self):
        invoice = get_object_or_404(
            Invoice,
            id=self.kwargs['pk'],
            subscription__user=self.request.user  # âŒ This won't work anymore
        )
        return invoice
```
...
class InvoiceDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_object(self):
        ...
        invoice = get_object_or_404(
            Invoice,
            id=self.kwargs['pk'],
            subscription__company=self.request.company  # âœ… Company-based
        )
        return invoice
...
<p>Overage Cost: â‚¹{{ overage_charge }}/month</p>
...
{% if not can_add_members %}
  <div class="alert alert-danger">
    ...
    You can add more members by paying â‚¹99/month per extra user,
