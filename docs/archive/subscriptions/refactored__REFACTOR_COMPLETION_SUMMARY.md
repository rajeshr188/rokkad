...
1. âœ… **Subscription** - Changed from `user` â†’ `company` ForeignKey
2. âœ… **Invoice** - Now company-level invoices
3. âœ… **Payment** - Track payments by company
4. âœ… **UsageMetrics** - Company resource usage tracking
5. âœ… **PriceOverride** - Enterprise custom pricing per company
6. âœ… **Plan** - Feature tiers (unchanged, still supports all models)
...
```
OLD:
User â†’ Subscription (One per user)
User â†’ invoices to email account

NEW:
Company â†’ Subscription (One per company)
Company â†’ invoices sent to company admins
  â”œâ”€ Owner manages billing
  â”œâ”€ Admins can be delegated billing permissions
  â””â”€ Staff/members consume seats (no payment responsibility)
```
...
def get_overage_charge()
    # Returns overage_users * plan.extra_user_price (â‚¹99/user)
...
```python
def is_active()
    # Check if override valid today (valid_from â‰¤ today â‰¤ valid_until)
```
...
| Field | Before | After |
|-------|--------|-------|
| `user` | OneToOneField(User) | âŒ Removed |
| `company` | âŒ Not present | OneToOneField(Company) âœ… |
| `current_user_count` | IntegerField | âŒ Removed (now method) |
| `current_transaction_count` | IntegerField | âŒ Removed (moved to UsageMetrics) |
| `current_invoice_count` | IntegerField | âŒ Removed (moved to UsageMetrics) |
| `razorpay_order_id` | âŒ Not present | CharField âœ… |
...
|-------|--------|-------|
| `razorpay_order_id` | âŒ Not present | CharField âœ… |
| Display (\_\_str\_\_) | "INV-{num} - {user.username}" | "INV-{num} - {company.name}" âœ… |

### UsageMetrics Model

| Field | Before | After |
|-------|--------|-------|
| Display (\_\_str\_\_) | "{user.username} - {date}" | "{company.name} - {date}" âœ… |
| New methods | None | `is_near_*_limit()` âœ… |
...
### 1. Company Creation
```
User signs up â†’ User created in public.user
User creates "Acme Corp" â†’ Company created in public.company
  â”œâ”€ Company auto-enters 14-day trial
  â”œâ”€ Subscription created: company=Acme, plan=Starter, status=trial
  â””â”€ Tenant schema created: acme_corp (for company data)
```

### 2. Member Invitation
```
Company owner invites staff@example.com
  â”œâ”€ CompanyInvitation created
  â”œâ”€ Staff receives email with accept link
  â””â”€ Staff accepts â†’ Membership created

Staff logs in:
  â”œâ”€ Selects "Acme Corp" workspace
  â”œâ”€ Accesses acme_corp tenant schema
  â””â”€ Uses company's subscription (no personal billing)
```

### 3. Billing Cycle
```
Day 0: Company created â†’ Trial (14 days)
Day 14: Trial ends â†’ Invoice created, subscription status â†’ past_due
Day 14-21: Owner sees payment prompt
Day 21: Owner upgrades to Professional plan
  â”œâ”€ Payment processed via Razorpay
  â”œâ”€ Subscription status â†’ active
  â”œâ”€ All members regain feature access
  â””â”€ Invoice marked as paid

Day 21-51: Company uses Professional plan
Day 51: Auto-renewal â†’ New invoice, payment attempted
  â””â”€ If success â†’ subscription renewed
  â””â”€ If failure â†’ status â†’ past_due (dunning starts)
```

### 4. Billing Manager Permissions
```
Acme Corp Membership:
  â”œâ”€ Owner (auto-billing-manager) â†’ Can manage billing
  â”œâ”€ Admin with role â†’ If role has 'manage_company_billing' permission â†’ Can manage
  â””â”€ General Member â†’ Cannot access billing

Only billing managers can:
  âœ“ View invoices
  âœ“ Upgrade/downgrade plan
  âœ“ View payment history
  âœ“ Manage renewal settings
```
...
- **Subscription** model: Major refactor (user â†’ company)
...
- Company creation â†’ auto-subscription
- Trial expiration
- Payment processing
- Member seat limits
...
|------|-------|-----------|
| Data loss | ðŸŸ¢ Low | No data deleted, just structure change |
| Downtime | ðŸŸ¢ Low | Can migrate with app running |
| Backward compat | ðŸ”´ High | Breaking change: `user` field removed |
| User impact | ðŸŸ¡ Medium | Views will 404 until updated |
...
âš ï¸ **These will not work after migration:**
...
- [ ] View tests pass
- [ ] Can create company â†’ auto-subscription created
...
### Quality
- âœ… Syntax validated (`python manage.py check` passes)
- âœ… Django best practices followed
- âœ… Timezone-aware (`timezone.now()` used everywhere)
- âœ… Verbose names for admin
- âœ… Proper Meta options (ordering, unique_together, indexes)
