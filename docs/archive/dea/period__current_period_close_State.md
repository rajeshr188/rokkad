...
These use the **previous periodâ€™s closing statement** as the next periodâ€™s opening balance.
...
## âš ï¸ What is only partially supported / simplified
...
## âŒ What is not currently built as a formal period-end subsystem
...
|---|---|---|
| Accrual adjustments | âš ï¸ Manual only | No dedicated pre-close workflow |
| Prepaid expense adjustments | âš ï¸ Manual only | No explicit automation/checklist |
| Depreciation posting | âš ï¸ Manual only | No depreciation engine seen |
| Interest accrual for loan system | âŒ Not formalized | Important gap for Girvi/loan correctness |
| Close income accounts | âœ… Yes | Through period close logic |
| Close expense accounts | âœ… Yes | Through period close logic |
| Transfer profit to retained earnings | âœ… Yes | Implemented |
| Reset P&L accounts to zero | âœ… Effectively yes | By closing entry logic |
| Carry forward balance sheet | âœ… Yes | Via opening/closing statement logic |
...
If you want, I can next **analyze the gaps and implement a proper â€œpre-close checklist + adjustments workflowâ€ for DEA**.
