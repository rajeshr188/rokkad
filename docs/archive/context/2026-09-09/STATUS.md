...
> Historical snapshot from docs/STATUS.md before the 2026-09-09 documentation cleanup.
...
## 2026-09-09 - Razorpay acceptance shelved; review cleanup reprioritized

At the owner's request, moved Razorpay setup and provider test-mode acceptance to
[FW-002](../../../plans/future-work.md#fw-002-razorpay-setup-and-provider-test-mode-acceptance).
The owner has not started provider setup. Existing billing implementation and mocked
regression coverage remain; no provider integration/testing was performed this turn.
Acceptance is still required before real paid onboarding, but does not block other
project improvements. No request for provider credentials or endpoint setup now.

Current next work is R08: correct active docs/dependency policy and separate current
status/memory from preserved history. Then address R09/R10 onboarding terminology,
unused legacy settings and obsolete boundary guards, followed by proven dead-code/
dependency cleanup and measured dashboard/routing/module improvements. Updated the
active plan and stable memory to prevent accidental resumption of shelved billing
work. This turn changes tracking documentation only; no runtime changes or tests.

## 2026-09-09 - Final billing review decisions

Added the paginated owner-only Billing reviews queue, linked from Billing. It shows
unresolved full refunds and unpaid checkouts (which may simply be abandoned).
Invoice review forms support final retain-access, end-current-refunded-term and
verified-stale-payment-return decisions. Actor, reason, displayed subscription
revision and before-state evidence are stored in immutable BillingResolution.

Ending access requires the latest completed checkout, a full immutable refund,
matching current plan/end date and an already-started, unexpired term. Newer or
future paid terms are protected. Stale issued checkouts can be resolved only with
provider-verified processed refunds covering the entire bound payment: payment and
refund history are recorded without activating old terms or changing the current
subscription. Identical decisions replay once; conflicts and stale reviews fail
closed. Final decisions remain visible on the invoice and leave the queue.

Validation: the initial 46 review/recovery/checkout tests passed, then all 12 final
review tests passed including concurrent decisions and cancellation/returned-payment
audit rollback. Coverage includes newer/future term protection, partial-refund denial,
revision and Workspace boundaries, immutable evidence, owner HTTP queue/POST and
payment replay after cancellation. Provider calls and receipt delivery were mocked.
Owner migrations 0008/0009 applied to development; runtime SELECT/INSERT grants,
system checks, migration drift and whitespace pass. No real money or emails sent.
Changes remain uncommitted with earlier hardening increments.

Next: provider test-mode acceptance on an HTTPS callback endpoint. Unknown/orphan
or legacy contracts without a frozen local checkout remain support investigations;
refund issuance, proration and chargebacks are not automated. See
[review guide](../../../flows/subscription-checkout.md#resolve-a-billing-review).
License scoping remains shelved.

## 2026-09-09 - Verified payment recovery and processed refunds

Added owner-authorized invoice recovery (Check provider records / Verify and apply)
and `reconcile_subscription_payment`, which defaults to a read-only provider check.
Both bind existing invoice/Workspace/order/payment/amount/currency, require a reason,
and reuse the locked capture path. Apply rechecks canonical ownership and appends
an actor/reason audit event. No force-paid or stale-revision bypass was added.

`refund.processed` fetches the exact provider refund and payment before recording
immutable PaymentRefund evidence. Unique refund IDs and Company/Invoice/Payment
locks prevent duplicate/concurrent totals; processed refunds cannot exceed payment.
Partial refunds retain access. Full refunds retain access pending owner review
(the conservative default, not automatic cancellation), and are visibly flagged on
the invoice. Original invoice/term evidence is preserved. Late captured callbacks
do not undo a refund. Provider calls are GET verification only; no refund issuance.

Validation: the initial 53 recovery/checkout/billing tests passed; after additional
cross-Workspace, audit rollback, invoice rendering and concurrent-refund coverage,
all 15 dedicated recovery tests passed. Provider I/O and receipts were mocked.
Owner migrations 0006/0007 applied to the development database, runtime refund-table
SELECT/INSERT grants verified, and runtime check/migration drift/whitespace pass.
No real payments, refunds or emails sent. Changes remain uncommitted.

Known limits remain explicit: unknown/orphan contracts, stale captures and refunds
without recorded capture history require a separately reviewed support resolution;
full-refund access-review resolution and chargebacks are not automated. Next: add
those reviewed resolution actions, then complete provider test-mode acceptance.
See [the recovery guide](../../../flows/subscription-checkout.md#recover-a-known-payment-or-refund).
License scoping remains shelved.

## 2026-09-09 - Paid subscription expiry

The canonical effective-billing policy now requires ACTIVE plus now < end_date for
paid access. At the exact end instant (or with a missing end), it derives EXPIRED
and recovery-only without writing subscription status, revision, payment events or
Workspace lifecycle. Middleware, service access and entitlement features/limits
share this decision; explicit capacity overrides do not bypass paid expiry.

Owner billing recovery stays available. The dashboard shows effective Expired state,
the actual period end and a renewal link. Reminders no longer claim automatic renewal.
A fresh captured purchase can renew an expired term, including another plan; early
active plan changes still require proration review. Replaying an old paid invoice
cannot restore expired access. Trial expiry policy is unchanged.

Validation: 51 subscription/checkout/request-boundary tests and 65 broader
control-plane/app-conformance checks passed (suites overlap). Added exact timestamp,
missing date, non-active status, entitlement override, owner recovery and renewal/
replay coverage. Runtime system check, migration drift and whitespace checks pass.
No schema migration or normal development data changes; provider I/O and receipt
email remained mocked. Changes are uncommitted with the prior increments.

Next: explicit payment reconciliation/refund recovery, followed by provider test-mode
acceptance. See [checkout flow](../../../flows/subscription-checkout.md) and
[hardening plan](../../../plans/project-hardening.md). License scoping remains shelved.

## 2026-09-09 - Workspace-bound subscription checkout

Implemented the R03/R04 checkout repair: distinct order/confirmation endpoints,
server-frozen Workspace/plan/cycle/paise/contact/entitlement evidence, unique order
and checkout identities, and a PostgreSQL evidence update guard. Canonical owner
and matching context are checked before browser provider I/O. Signature plus fetched
captured-payment order/amount/currency validation precedes one locked transaction
for invoice, Payment, subscription terms, entitlement projection and audit events.
New unpaid subscriptions no longer silently become trials. Repeat/concurrent
captures do not extend twice or schedule duplicate receipts. Receipts use the saved
billing contact after commit. Invoice HTML/PDF no longer reference subscription.user.

Webhooks use the configured secret, constant-time raw-body HMAC, the real payment
entity shape, locked event replay checks and the same capture service. Failed
attempts do not downgrade paid access; stale/unknown/refund events remain visible
for reconciliation. One-off renewals disable auto-renew and freeze the purchased
month/year. Active plan switches require proration review. See [the flow and known
limits](flows/subscription-checkout.md) and [ADR](../../../adr/2026-09-09-workspace-checkout-evidence.md).

Validation: 20 checkout tests pass, including real HTTP order/confirmation and global
anonymous signed callback routes, cross-Workspace denial, immutable evidence,
rollback, saved prices/contacts, monthly/yearly renewal, provider-field mismatches,
replay, after-commit receipt and concurrent captures. In the 85-test combined run,
65 other subscription/control-plane/ownership/MVP tests passed; its one new callback
fixture failure was corrected and all 20 checkout tests rerun successfully. Provider
I/O and email were mocked. Migration drift/system/whitespace checks pass. The normal
development database had no duplicate non-null provider order IDs; owner migrations
0004/0005 applied successfully. No real provider payment or email was sent.

Changes remain uncommitted with the preceding foundation/operator work. This is not
commercial-launch acceptance. Next: paid-period expiry in canonical billing policy,
then operator reconciliation/refund recovery and provider test-mode acceptance.
License scoping remains shelved; no production configuration was deployed.

## 2026-09-09 - Explicit Workspace operator commands

Completed R05 for `seed_default_loan_products`, `check_loan_document_integrity` and
`dispatch_pawn_loan_notices`. Each requires --workspace-id, rejects invalid/missing/
unknown/conflicting identity, and opens/clears its own RLS context. Product seeding
and queued delivery require ACTIVE at entry. Read-only integrity checks remain
available for inactive Workspace recovery. Dispatch validates ISO datetimes, treats
naive values in the configured timezone and limits batches to 1–1000. Existing
service permissions, internal bootstrap/delivery behavior and notice failure-count
semantics remain unchanged. See [the operator guide](../../../implementation/loans-operator-commands.md).

Validation: all 25 focused command/product/notice tests passed. New coverage includes
a fresh Python process operating through a restricted SQL role with no ambient
Workspace, database/Python context cleanup, scoped/idempotent seeding, rollback,
missing/conflicting IDs, lifecycle and timestamp/limit validation. System checks and
whitespace pass. No real notices sent, normal development data changed, schema
migrations performed or production configuration deployed. Changes remain uncommitted,
alongside the prior foundation checkpoint. Next: R03/R04 paid billing correctness.

## 2026-09-09 - Reproducible CI/container foundation

Implemented the first approved [hardening increment](../../../plans/project-hardening.md):
replaced schema-tenancy CI with Workspace/RLS checks on PRs/main/rls-mvp; added a
Python 3.14 multi-stage non-root image and a source allowlist for its build context;
replaced development Compose with PostgreSQL 16 and separate owner/runtime roles;
added a separate production recipe. Production now requires explicit runtime
credentials and non-wildcard hosts. The web launcher checks forced RLS/runtime role
and pending migrations before serving; migrations remain an explicit owner-only
operation. The provisioning script creates only a new login and refuses existing
roles. See [Containers and CI](../../../implementation/container-and-ci.md).

Validation: 90 foundation/authorization/first-loan tests passed, plus the final six
deployment-entrypoint tests. A clean Linux image build exposed missing C headers
for psycopg2; adding libc6-dev to the build stage resolved it. Image pip check passed;
verified non-root execution, required source presence and exclusion of local env,
Git, virtualenv, media and backup paths. Development Compose syntax passed.
After explicit approval for the disposable database login, fresh owner migrations
passed, runtime startup passed and owner startup failed with tenancy.E020. An
isolated web process returned HTTP 200; production collectstatic copied 310 files
and post-processed 802. Temporary rokkad-review-smoke containers/volumes were removed.
The normal development database was not migrated or changed by this rehearsal.

TLS/HSTS, placeholder-secret and debug-toolbar warnings in the disposable settings
were not silenced or presented as production acceptance. Actual deployment/private
storage/restore acceptance remains pending. GitHub-hosted execution awaits a push;
no image was published, and this checkpoint is uncommitted. Next: R05 operational
commands, then billing correctness. License scope remains shelved.

## 2026-09-09 - Project architecture and residue review

Reviewed checkpoint `4b08c3f`; findings and proposed delivery order are in the
[project review](../../../architecture/2026-09-09-project-review.md). Keep Workspace/RLS and
the modular monolith. Priority findings cover stale schema-tenancy CI/container
recipes, incomplete payment/order/Workspace binding and billing legacy attributes,
three CLI commands missing explicit Workspace context, deployment/media/restore
acceptance, Loans response URL rewriting, contradictory current docs, retired tour
choices/settings and dashboard performance/incomplete totals. Cleanup candidates
are distinguished from required aliases, redirects and historical evidence.

Read-only introspection reproduced the PaymentView method-decorator error and the
unscoped document-integrity error; confirmed missing command arguments and absent
Subscription.user/webhook-secret settings. No payments or notifications were sent.
The explicit-module validation run passed 66 tenancy/access/billing tests; the first
package-label invocation had two test-discovery import errors. `pip check` passed.
This is targeted architectural verification, not a production or exhaustive security
audit. Only review documentation changed; no application code/schema/business data
changes. Recommended next: replace CI/build recipes and verify runtime-role gates,
then repair operational commands and paid billing before relying on those paths.
Recommendations remain proposed; license scope is still shelved under FW-001.
...
Historical assessments are preserved in [archive/root](../../root).
