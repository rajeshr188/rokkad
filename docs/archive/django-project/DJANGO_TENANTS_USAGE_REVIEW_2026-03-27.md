...
## What â€œCore Tenant Resolution Middleware Is Bypassedâ€ Means
...
In current runtime behavior, schema is mainly selected from the userâ€™s selected workspace in profile. That means domain resolution is not the primary gate for schema assignment during request handling.
...
### Step 1 â€” Add the Tenant Test Runner to settings
...
### Step 2 â€” Create a shared base class
...
### Step 3 â€” Migrate tests incrementally

**Target files in priority order:**

1. `apps/tenant_apps/girvi/tests/test_series_guardrails.py` â€” 17 DB tests, primary target. Remove the `Customer` FK setup that was ported from the old `TestCase` setUp; the guardrail tests do not require a Customer record at the series level.

2. `apps/tenant_apps/girvi/tests.py` â€” currently a stub. Expand here once the base class is verified.

3. `apps/tenant_apps/girvi/tests/test_payment_integration_pr1_pr2.py` â€” uses `SimpleTestCase` with mocks; does NOT need migration to `TenantTestBase` (no real DB access).
...
### Step 4 â€” Validate in isolation first, then combined
...
# Step B: combined with orgs tests â€” this is what was failing before
...
### Phase 2 (completed 2026-03-28) â€” Four findings remediated

**Finding #1 â€” Deferred (infrastructure-gated)**
...
**Finding #2 â€” Middleware routing enforcement hardened** (`apps/orgs/middleware_v2.py`)
...
**Finding #3 â€” Tenant context source-of-truth standardized** (`apps/orgs/tenant_context.py`)
...
**Finding #4 â€” Company lifecycle policy formalized** (`apps/orgs/models.py`, `apps/orgs/admin.py`, `apps/orgs/views.py`)
- `auto_drop_schema = False` (was `True`); schema drop is now explicit and guarded.
- `archive()`: soft-delete â€” sets `is_deleted=True`, saves with `update_fields`.
...
### Phase 3 (completed 2026-03-28) â€” Decorator consolidation
- `apps/orgs/decorators.py` deleted.
- All functionality (live and useful) merged into `apps/orgs/decorators_v2.py`:
  - `permission_required` â€” fine-grained; checks `role.permissions` M2M by codename; audit-logged.
  - `any_permission_required` â€” OR variant of above.
  - `object_permission_required` â€” Guardian object-level permission check.
  - `roles_required` â€” coarse role-name gate; migrated from old file; rewritten with `select_related`, `PermissionDenied`, and public/tenant schema handling.
  - `workspace_required` â€” lightweight "must have a resolved workspace" gate.
