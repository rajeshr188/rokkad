...
Iâ€™m going to do a targeted codebase review of how django-tenants is configured and used today, then map gaps against package capabilities and give concrete, prioritized improvements with file-level references. Iâ€™ll start by locating tenant models, settings, middleware, routing, and tenant-aware access control flow.
...
Iâ€™ve confirmed core settings and the custom security middleware. Next Iâ€™m checking request routing and schema selection flow to verify whether django-tenants middleware lifecycle is correctly preserved or partially bypassed.
...
I found additional schema-switch hints but the automated extraction had path resolution issues, so Iâ€™m now validating those locations directly in the workspace to avoid false positives and produce a reliable review.
...
Iâ€™m now reviewing onboarding and rate/membership middleware interactions, because these are where tenant bootstrap and cross-schema consistency issues usually surface.
...
I now have enough core app context; next Iâ€™m scanning tenant-aware management commands and tests to identify operational and migration gaps in how schemas are targeted outside requests.
...
- This means tenant selection is driven by user profile state (middleware_v2.py) instead of domain mapping, which weakens django-tenantsâ€™ intended request isolation model.
...
**What Youâ€™re Already Doing Well**
...
- If you intentionally chose profile-driven switching over domain-driven routing, I can give a separate â€œworkspace-switch-firstâ€ hardening design that still stays safe with django-tenants.
...
Iâ€™ll create a dedicated markdown report under django_project/docs capturing the findings, and then Iâ€™ll answer your three architecture questions with concrete examples from your current flow. Iâ€™m first checking whether the docs folder already exists so I can add the file in the right place.

Read [](file:///c%3A/Users/rajes/OneDrive/Desktop/rokkad/django_project)

I confirmed django_project/docs is not present, so Iâ€™m creating it and writing a structured review document there with both findings and guidance on tenant resolution approaches.
...
What â€œcore tenant resolution middleware is bypassed in runtimeâ€ means:
- In standard django-tenants flow, each request resolves tenant from the incoming host/domain first, then switches schema.
- In your active runtime flow, schema selection is primarily done from the userâ€™s selected workspace in profile via middleware_v2.py, instead of domain-first resolution.
...
Right now your onboarding (views.py) creates a blank schema â€” no seed data. Clone would replace that with a fully configured starting point.

3. Other django-tenants Features Not Being Used
schema_context() â€” partially used, could be used more systematically
You use it in views.py (company delete, subscription check) and views.py (team creation). But in models.py line 29 there's a commented-out tenant_context(...) call â€” suggests hesitation. This is the right tool for any code that needs to momentarily query a different schema in a task/signal/celery worker.

TenantTestCase / FastTenantTestCase â€” not used at all.
Your tenant app tests (tests_pr9.py) manually call schema_context(self._schema_name) â€” that's fragile. TenantTestCase creates a real tenant + domain per test class in a transaction, provides self.tenant, isolates properly, and cleans up. FastTenantTestCase wraps each test in a savepoint instead of creating/dropping schemas, so it's 10x faster.

tenant_command / all_tenants_command wrappers â€” not used.
...
PG_EXTRA_SEARCH_PATHS â€” not configured.
Lets you add extra schemas (e.g. a shared_data schema with read-only reference tables â€” gold rates, standard terms, product catalogue templates) that are visible within every tenant schema via Postgres search_path. This means you don't duplicate reference data across 50 tenant schemas.

TENANT_LIMIT_SET_CALLS â€” not set.
...
create_missing_schemas â€” not wired into CI/deploy.
...
SESSION_COOKIE_DOMAIN / CSRF_COOKIE_DOMAIN â€” commented out in your settings.
...
Iâ€™ve confirmed the duplication surface: urls.py and tenant_urls.py repeat most routes. Iâ€™ll now extract a shared pattern list and have both URLconfs compose from it while keeping their schema-specific routes separate.
...
```
URL dedup refactor is applied to active URLconfs. Iâ€™ll now run diagnostics and Django checks/tests to confirm behavior is unchanged.
...
Item 1 is done and validated. Iâ€™ll move to item 2 now: find Girvi/DEA tests using manual `schema_context` and migrate them to `TenantTestCase`/`FastTenantTestCase` where practical.
...
I found no `schema_context` usage in Girvi/DEA tests to replace directly, so for item 2 Iâ€™ll still harden the active Girvi DB tests by moving them to `FastTenantTestCase` and removing non-essential setup that depended on profile/workspace assumptions.
...
Item 2 is complete and passing for Girvi (`FastTenantTestCase` migration). Iâ€™ll now implement item 4 and item 5 as safe configuration changes in settings: enable `TENANT_LIMIT_SET_CALLS` and add configurable `PG_EXTRA_SEARCH_PATHS`.
...
Settings updates for items 4 and 5 are in and validated. Next Iâ€™ll tackle item 3 by wiring onboarding to optionally clone from a template tenant schema, with safe fallback to normal schema creation if cloning isnâ€™t configured.
...
For clone-based onboarding, I verified the installed django-tenants API supports `CloneSchema().clone_schema(...)`; Iâ€™ll now add a safe helper in onboarding that clones from a configured template schema when available, otherwise falls back to normal schema creation.
...
I hit an issue: the `FastTenantTestCase` migration for the Girvi guardrails file is not stable in this repoâ€™s current test setup (`girvi_license` table missing at runtime), so Iâ€™m rolling that specific test file back to keep the suite safe; we can reintroduce tenant test base later via a dedicated test runner path.
...
- Result: shared routes now live in one place, so adding/removing common routes wonâ€™t diverge between public and tenant URLconfs.
...
- I attempted migrating girvi guardrail tests to FastTenantTestCase, but this became unstable in this repoâ€™s current runner setup (tenant table relation errors), so I safely rolled that file back to avoid introducing regressions: test_series_guardrails.py
