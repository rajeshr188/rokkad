...
â”‚ Onboarding   â”‚      â”‚ Workspace      â”‚    â”‚ Dashboard      â”‚
â”‚ (New User)   â”‚      â”‚ Selection      â”‚    â”‚ (Existing)     â”‚
...
**Possible Outcomes:**
1. â†’ `/onboarding/start/` (incomplete onboarding)
2. â†’ `/workspace/create/` (no memberships)
3. â†’ `/dashboard/` (workspace selector)
4. â†’ `/billing/renew/` (expired subscription)
5. â†’ `/workspace/dashboard/` (success)
...
â”‚ [Logo] Rokkad  ðŸ“¦ [Workspace: ABC Corp â–¼]    [ðŸ‘¤ User Menu â–¼] â”‚
...
**Data Section:**
- ðŸ’° Loans (Girvi) (if `data_view`)
- ðŸ“ˆ Sales (if `data_view`)
- ðŸ›’ Purchase (if `data_view`)
- ðŸ§® Accounting (DEA) (if `data_view`)
- ðŸ‘¥ Contacts (if `data_view`)

**Admin Section:**
- âš™ï¸ Settings (if `workspace_settings`)
- ðŸ’³ Billing (if `billing_view`)
- ðŸ“Š Reports (if `reports_view`)
...
**User Actions:**
1. View all workspaces
2. Click "+ Create New Workspace" â†’ Modal opens
3. Fill form â†’ Submit â†’ Workspace created
4. Click card â†’ View details
...
â”‚  [Icon] ABC Corporation                    [Edit Workspace]â”‚
â”‚  ðŸ“… Created Mar 15, 2026  ðŸ‘¤ Owner: John Doe              â”‚
â”‚                                                            â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”      â”‚
â”‚  â”‚ Team Members â”‚ â”‚ Invitations  â”‚ â”‚ Your Role    â”‚      â”‚
â”‚  â”‚     12       â”‚ â”‚      3       â”‚ â”‚   [Owner]    â”‚      â”‚
...
- **Edit Role (HTMX):**
  ...
  - Select new role â†’ Auto-saves via HTMX
...
#### Page Elements
- **Breadcrumbs:**
  - Workspaces â†’ Create (or Company Name â†’ Edit)
...
- **Action Buttons:**
  - [Cancel] â†’ Back to workspace list
  - [Create Workspace] or [Save Changes] â†’ Submit

- **What Happens Next Box (Create only):**
  ```
  âœ“ You'll be assigned as Owner
  âœ“ Invite team members with roles
  âœ“ Set up business preferences
  âœ“ Start managing data
  ```
...
- **Action Buttons:**
  - [Cancel] â†’ Back to team tab
  - [Send Invitation] â†’ Submit (HTMX)
...
1. **User clicks email link** â†’ Redirected to accept page
...
3. **Accept Page UI:**
   ...
   â”‚                                             â”‚
   â”‚  [Company Icon] ABC Corporation             â”‚
   â”‚                                             â”‚
   â”‚  Invited by: John Doe                       â”‚
   â”‚  Role: [Admin Badge]                        â”‚
   â”‚  Expires: Mar 25, 2026                      â”‚
   â”‚                                             â”‚
...
**UI Features:**
- **Page Header:**
  - Breadcrumbs: Dashboard â†’ Loans
  - Title: "Loans (à¤—à¤¿à¤°à¤µà¥€)"
...
- **Billing Alert Banner:**
  ```
  âš ï¸ Your subscription expired on Mar 15, 2026.
...
1.  SIGNUP â†’ Email Verification â†’ Profile Setup
2.  ONBOARDING â†’ Profile â†’ Workspace â†’ Team â†’ Tour â†’ Complete
3.  LOGIN â†’ Smart Landing â†’ Workspace Selection â†’ Dashboard
4.  DAILY USE â†’ Navigation (permission-filtered) â†’ Data Operations
5.  TEAM MGMT â†’ Invite â†’ Accept â†’ Manage Roles â†’ Remove
6.  WORKSPACE MGMT â†’ Create â†’ Switch â†’ Edit â†’ Delete (Owner only)
7.  SUBSCRIPTION â†’ Active Check â†’ Warning â†’ Renewal â†’ Reactivation
8.  SECURITY â†’ Membership Validation â†’ Audit Logging â†’ Access Control
9.  LOGOUT â†’ Session End â†’ Audit Log â†’ Redirect to Login

Every step:
âœ“ Permission-validated
âœ“ Audit-logged
âœ“ Subscription-protected (where applicable)
âœ“ Membership-validated
âœ“ Role-aware UI
```
