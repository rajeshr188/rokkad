...
#### Models
âœ… [apps/tenant_apps/contact/models.py](apps/tenant_apps/contact/models.py)
- Changed `first_name` â†’ `firstname`
- Changed `last_name` â†’ `lastname`
...
#### Filters
âœ… [apps/tenant_apps/contact/filters.py](apps/tenant_apps/contact/filters.py)
- Updated `universal_search()` to use `firstname__icontains` and `lastname__icontains`

#### Admin
âœ… [apps/tenant_apps/contact/admin.py](apps/tenant_apps/contact/admin.py)
- Updated `search_fields` to use `firstname`, `lastname`
- Updated `list_display` to use `name` (which is now a property)

#### Forms
âœ… [apps/tenant_apps/contact/forms.py](apps/tenant_apps/contact/forms.py)
- Updated AddressForm: `doorno` â†’ `door_number`, `zipcode` â†’ `zip_code`
- Updated ProofForm: `proof_no` â†’ `proof_number`, `doc` â†’ `document`

#### Girvi App (Related Files)
âœ… [apps/tenant_apps/girvi/admin.py](apps/tenant_apps/girvi/admin.py)
- Updated search fields: `customer__firstname`, `customer__lastname`

âœ… [apps/tenant_apps/girvi/filters.py](apps/tenant_apps/girvi/filters.py)
- Updated universal search: `customer__firstname`, `customer__lastname`

âœ… [apps/tenant_apps/girvi/services.py](apps/tenant_apps/girvi/services.py)
- Updated values clause: `customer__firstname`, `customer__lastname`

âœ… [apps/tenant_apps/girvi/views/reports.py](apps/tenant_apps/girvi/views/reports.py)
...
âœ… [apps/tenant_apps/contact/migrations/0002_contact_model_refactoring.py](apps/tenant_apps/contact/migrations/0002_contact_model_refactoring.py)

**Key Operations**:
1. Renames `name` field â†’ `firstname`
...
- `firstname` âœ…
- `lastname` âœ…  
- `doorno` â†’ `door_number` (snake_case for clarity)
- `zipcode` â†’ `zip_code` (snake_case for clarity)
- `proof_no` â†’ `proof_number` (snake_case for clarity)
...
1. âœ… **Simpler field names**: `firstname`, `lastname` (not snake_case)
2. âœ… **No redundancy**: Single source of truth for name data
3. âœ… **Backward compatibility**: `customer.name` still works as a computed property
4. âœ… **Better performance**: No need to keep three fields in sync
5. âœ… **Cleaner code**: One field per piece of data
...
âš ï¸ **If you have data in both `name` and `firstname` fields**:
