...
Loans rewrite Phase 1, `E2.1`, and `E2.2` are implemented. The registered tenant app has pure PawnLoan vocabulary/policy contracts and tenant migration `0001_initial` for `LoanLicense`, `LoanSeries`, `LoanNumberSequence`, `PawnLoan`, `PawnCollateralItem`, `LoanPolicySnapshot`, and `LoanChangeLog`. Licenses and loans carry explicit workspace ownership and reject workspace values that do not match the active tenant; borrowers link directly to tenant `party.Party`. License/series setup now goes through tenant-only services, issuance eligibility fails for inactive or expired setup, and sequence configuration changes are public-workspace audited. Locked allocation, Loans URLs/UI, loan workflow services, accounting events, repayments, accruals, releases, FundingLoan, and outbox runtime do not exist yet.
