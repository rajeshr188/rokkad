...
Loans rewrite Phase 1 and `E2.1`-`E2.3` are implemented. The registered tenant app has pure PawnLoan vocabulary/policy contracts and tenant migration `0001_initial` for `LoanLicense`, `LoanSeries`, `LoanNumberSequence`, `PawnLoan`, `PawnCollateralItem`, `LoanPolicySnapshot`, and `LoanChangeLog`. Licenses and loans carry explicit workspace ownership and reject workspace values that do not match the active tenant; borrowers link directly to tenant `party.Party`. License/series setup goes through tenant-only services, issuance eligibility fails for inactive or expired setup, and sequence configuration changes are public-workspace audited. Pawn-loan and release number allocation uses non-consuming preview plus atomic sequence-row locking; committed numbers never recycle and exhaustion requires another series. The E2.5 draft service must call `allocate_pawn_loan_number` inside draft creation. Loans URLs/UI, loan workflow services, accounting events, repayments, accruals, releases, FundingLoan, and outbox runtime do not exist yet.
