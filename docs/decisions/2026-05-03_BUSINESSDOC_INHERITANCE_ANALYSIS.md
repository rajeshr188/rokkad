# ADR: BusinessDoc Inheritance — Analysis and Decoupling Plan
...
accounting entries (vouchers → journal entries → ledger/account transactions).
...
### PaymentVoucher and JournalEntryVoucher — correct ✅
...
### BaseLoan / Loan / LoanPayment — incorrect ❌
...
- Gains only the 4 audit fields (`created_at/by`, `updated_at/by`) — which are
...
the composition link — the loan → payment → voucher → JE chain is already
...
- [x] Run `scripts/check_dea_boundary.py` — 0 violations (2026-05-03)
- [x] Run `manage.py makemigrations` — `0020_remove_auto_post_to_accounting.py` generated; `--check` passes after
