...
- Girvi Phase 3 task 28 is in progress with forms-to-workflow extraction slices: `LoanItemStorageBoxForm` no longer performs range-assignment side effects in `save()` (moved to `service_modules/storagebox_workflow.py`), and release bulk formset workflow checks for duplicate/stale rows are now centralized in `service_modules/release_form_validation.py` while `ReleaseForm.save()` override was removed so the form remains validation-only.
