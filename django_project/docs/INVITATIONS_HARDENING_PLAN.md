...
**Location**: [apps/orgs/signals.py](../apps/orgs/signals.py#L13) — `@receiver(invite_accepted)` handler
...
**Location**: [apps/orgs/signals.py](../apps/orgs/signals.py#L53) — `@receiver(user_signed_up)` handler
...
**Impact**: no orphaned pending invitations; all pending → membership on signup

---

#### Step 1.3: Remove duplicate membership creation from `CustomAcceptInvite` view

**Location**: [apps/orgs/views.py](../apps/orgs/views.py#L300) — `CustomAcceptInvite.get()` method

**Changes**:
- Delete the entire if-block at lines 310–313: the `Membership.objects.create()` call
...
**Location**: [apps/orgs/models.py](../apps/orgs/models.py#L285) — `PendingInvitation` Meta class
...
- Package URL: `/invitations/accept-invite/<key>/` → routed to package's `AcceptInvite`
- Custom URL: `/team/invitations/accept/<key>/` → routed to `CustomAcceptInvite`
...
class InvitationSignalTests(SimpleTestCase):
    ...
    def test_existing_user_accepts_invite_creates_membership(self):
        # User exists, invitation link accepted → membership created once
    ...
    def test_non_user_accepts_invite_creates_pending_invitation(self):
        # Non-user opens invite link → pending invitation created
        # No membership yet
...
- `apps/orgs/signals.py` — two signal handlers: `create_membership`, `create_membership_on_signup`
- `apps/orgs/views.py` — `CustomAcceptInvite` class (line 300)
- `apps/orgs/models.py` — `Membership` (L154), `PendingInvitation` (L285)
- `apps/orgs/urls.py` — accept invite endpoints at lines 54, 110
- `apps/orgs/tests.py` or new `apps/orgs/test_invitations.py` — test suite
...
1. **Phase 1** (Idempotency): steps 1.1 → 1.2 → 1.3 (sequential, can't skip)
...
- Migration of existing pending invitations (data cleanup) — defer if causes backward compat issues
- Social signup auto-provisioning from invite — handle in allauth plan if needed
- Invitation expiry enforcement — already in package; verify works
