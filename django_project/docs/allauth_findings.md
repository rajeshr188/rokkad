...
|---|---|---|
| `allauth.account` | ✅ Active | Email+username login, signup, logout |
| `allauth.socialaccount` + Google | ✅ Active | OAuth2/PKCE, profile picture sync |
| Custom templates | ✅ Partial | login, logout, signup, password reset/change/set overridden |
| `social_account_added` / `updated` signals | ✅ Active | Saves Google profile picture to `UserProfile` |
| `invite_accepted` signal (django-invitations) | ✅ Active | Creates `Membership` on accept |
| `ACCOUNT_ADAPTER` | ✅ Set | `InvitationsAdapter` — allauth adapter delegates to invitations |
| `INVITATIONS_INVITATION_MODEL` | ✅ Set | `orgs.CompanyInvitation` |
| Management command `fetch_google_profile_pics` | ✅ Exists | Backfill for existing users |
...
| **`ACCOUNT_SIGNUP_FIELDS` typo** — missing comma between `"username*"` and `"password1*"` | settings/base.py | `["email*", "username*", "password1*"]` |
| **`user_signed_up` signal is commented out** in signals.py | signals.py | Pending invitation → membership flow is incomplete; users who sign up after accepting invite never get their membership created |
...
2. **Custom `AccountAdapter`**  
   You have `InvitationsAdapter` but no project-specific adapter. A custom adapter overriding `is_open_for_signup()` would let you gate signups to invited users only — critical for a private SaaS.

3. **`user_signed_up` signal** (currently commented out)  
   The `PendingInvitation` → `Membership` flow on post-signup is partially written but disabled. This is a real gap: if someone accepts an invite before they have an account, their membership is never created.

4. **`ACCOUNT_USERNAME_REQUIRED = False`** + email-only auth  
   You have both email and username in signup fields. For a B2B SaaS, email-only is cleaner and has less friction. You already have `ACCOUNT_LOGIN_METHOD = {"email"}` — the username field in signup is redundant.

5. **`SocialAccountAdapter`** override  
   Would let you auto-assign workspace from invitation on Google OAuth signup — currently Google signups bypass the `invite_accepted` flow entirely.
...
3. Uncomment and fix `user_signed_up` → PendingInvitation → Membership flow
