I’ll trace the full invite lifecycle in code (models, signal registration, invitation acceptance flow) and then give you a concrete edge-case review with fixes/tests prioritized by risk.
...
I’ve confirmed signals are loaded via `OrgsConfig.ready()`. Next I’ll inspect the invite accept path and where each signal fires so the behavior is exact, not hypothetical.
...
I found where both signals are emitted in third-party code; next I’ll verify invitation settings defaults so we can reason about your exact runtime flow.
...
3. Your app’s combined flow today  
