...
### 1. Fix `ACCOUNT_SIGNUP_FIELDS` typo — missing comma between `"username*"` and `"password1*"`
...
1. Add context processor function to expose `GOOGLE_CLIENT_ID` from Django settings
   ...
   - Function: `def google_oauth_context(request)` — return `{'GOOGLE_CLIENT_ID': settings.SOCIALACCOUNT_PROVIDERS.get('google', {}).get('CLIENT_ID', '')}`
...
- `django_project/settings/base.py` — allauth settings, ACCOUNT_SIGNUP_FIELDS, SOCIALACCOUNT_PROVIDERS
- `django_project/context_processors.py` — new/updated context processor for Google client ID
- `templates/account/login.html` — hardcoded client_id at line 30
- `templates/account/signup.html` — hardcoded client_id at line 19
- `templates/account/password_*.html` — other account templates (do not modify; use custom if needed)

---

## Verification

- [ ] `python manage.py check` — no settings errors
