...
from django_tenants.utils import remove_www, get_public_schema_name, get_tenant_types, has_multi_type_tenants, get_tenant_domain_model, get_public_schema_urlconf
...
class WorkspaceMiddleware(TenantMainMiddleware):
    ...
    def process_request(self, request):
        ...
        if request.user.is_authenticated and request.user.workspace:
            ...
            connection.set_tenant(request.user.workspace)  # Set connection to the workspace's schema
            self.setup_url_routing(request)
            return None
