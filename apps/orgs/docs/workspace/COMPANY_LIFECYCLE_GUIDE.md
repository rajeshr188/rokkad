...
[non-existent]
  ...
  is_deleted=False                                    is_deleted=True
      ...
      └─────────── restore() ───────────────────────────────┘
                                                            │
                                                            ▼ (requires ALLOW_COMPANY_HARD_DELETE=True)
                                                    [permanently deleted]
...
2. A `Company` row is saved — `TenantMixin.save()` translates `auto_create_schema = True` into a `CREATE SCHEMA` call.
...
After schema creation the schema is empty — seed it before using it:
...
Select archived companies → **"Restore selected companies"** action.
...
No schema changes occur — the data was never touched.
...
`auto_drop_schema` is `False` by default — `hard_delete()` explicitly sets it to `True` immediately before calling `super().delete()` so the schema drop only happens on that explicit path.

### Via the admin

Select companies → **"Permanently delete selected companies"** action.
...
Audit logs are **immutable** — `AuditLogAdmin` has no add, change, or delete permission.

---

## 8. Caveats & Gotchas

- **Schema rename is not supported** — `schema_name` is set once from the company name and never updated. Rename requires a manual `ALTER SCHEMA` in PostgreSQL plus updating the `Company` row.
- **`auto_drop_schema = False` by default** — prevents accidental schema drops via Django ORM signals or partial deletes.
- **`Company.objects` hides archived companies** — any queryset that should include soft-deleted records must use `Company.all_objects`.
- **Hard delete cascades** — Membership, Domain, CompanyPreferenceModel, AuditLog rows referencing the company are removed by the database CASCADE. Media files are **not** automatically purged.
- **Public schema is indestructible** — both `archive()` (it's a no-op guard on the model) and `hard_delete()` raise or are blocked for `schema_name == "public"`.
- **Seeding is not automatic** — unless `TENANT_AUTO_SEED_ON_CREATE=True`, a freshly created tenant schema has no default data. Run `seed_tenant_defaults` manually.
