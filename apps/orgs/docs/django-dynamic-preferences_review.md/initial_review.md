...
|---|---|---|
| App install | `dynamic_preferences` is in base.py | ✅ |
| Per-company storage | `CompanyPreferenceModel` in models.py + registry in registries.py | ✅ |
| Registration | wired in apps.py via `preference_models.register(...)` | ✅ |
| Runtime helper | `CompanyPreferences` in preferences.py merges company override + global fallback | ✅ |
| UI | `CompanyPreferenceBuilder` in views.py + company_preferences.html | ✅ |
| Actual business use | mainly in loan.py for `loan_default_date` and metal interest-rate defaults | ⚠️ limited |
...
For finance-sensitive preferences, “who changed what and when” matters.  
...
> **Yes, `django-dynamic-preferences` is the right fit for your project — but only for company-level configurable defaults and policy toggles.**
