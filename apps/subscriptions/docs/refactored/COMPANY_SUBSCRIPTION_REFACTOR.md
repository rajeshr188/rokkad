...
**Added**:
- `company` field → OneToOneField to Company (the paying entity)
...
- Updated `__str__` to show company name: `"{invoice_number} - {company.name} - ₹{amount}"`
...
### User Signs Up
1. User creates account → User created in `public.user`
2. User creates "Acme Corp" company → `public.company` created
3. Company auto-enters **14-day trial** → `public.subscription` created with `company_id`
4. Tenant schema `acme_corp` created for company data

### User Invites Staff
1. Company owner invites staff email → `CompanyInvitation` created
2. Staff accepts → `Membership` created with role (owner/admin/member)
...
5. **Payment fails**: Company goes into `past_due` status → all members see restricted features
...
if not subscription.can_add_member():
    # Show upgrade prompt
    # Company must upgrade or pay overage for ₹99/month per extra user
```
...
- [ ] Create company → subscription auto-created in trial
- [ ] Trial active for 14 days
- [ ] End of trial → status changes to `past_due`
