"""Backend service for MVP unfixed commodity sale posting.

Unfixed sale records metal issue/exposure without creating final monetary
receivable or revenue. Rate fixing is responsible for later financial
settlement.
"""
...
UNFIXED_SALE_VOUCHER_TYPE = "COMMODITY_UNFIXED_SALE"
UNFIXED_SALE_MOVEMENT_EVENT_TYPE = "unfixed-sale-issue"
UNFIXED_SALE_EXPOSURE_EVENT_TYPE = "unfixed-sale-exposure"
UNFIXED_SALE_FINGERPRINT_VERSION = "commodity-unfixed-sale-v1"


@dataclass(frozen=True)
class UnfixedSalePostingPayload:
    source: Model
    sale_date: date
    party: Party
    commodity: Commodity
...
@dataclass(frozen=True)
class UnfixedSalePostingResult:
    created: bool
    voucher: Voucher
...
@transaction.atomic
def post_unfixed_sale(
    payload: UnfixedSalePostingPayload,
    *,
    actor=None,
) -> UnfixedSalePostingResult:
    """Post an unfixed sale as commodity issue plus open exposure only."""

    _validate_payload(payload)
    _validate_period(payload.sale_date)
    ...
    if existing_movement and existing_exposure:
        if existing_movement.voucher_id != existing_exposure.voucher_id:
            raise ValidationError(
                "Existing unfixed sale movement/exposure are linked to different vouchers."
            )
        return UnfixedSalePostingResult(
            created=False,
    ...
    if existing_movement or existing_exposure:
        raise ValidationError(
            "Incomplete existing unfixed sale posting found. "
            "Manual verification is required before retrying."
        )

    voucher_type = _unfixed_sale_voucher_type()
    ...
    if Voucher.objects.filter(
        ...
        raise ValidationError(
            "A posted unfixed sale voucher already exists for this source. "
    ...
    fingerprint = compute_fingerprint(
        ...
        UNFIXED_SALE_FINGERPRINT_VERSION,
    )
    voucher = Voucher.objects.create(
        ...
        narration=payload.narration or "Unfixed commodity sale",
    )
...
def _validate_payload(payload: UnfixedSalePostingPayload) -> None:
    errors = {}
...
def _unfixed_sale_voucher_type() -> VoucherType:
    voucher_type, _created = VoucherType.objects.update_or_create(
        name=UNFIXED_SALE_VOUCHER_TYPE,
        defaults={"description": "Unfixed commodity sale posting"},
    )
    return voucher_type


def _validate_period(sale_date: date) -> None:
    period = AccountingPeriod.objects.get_period_for_date(sale_date)
    if not period:
        raise ValidationError(f"No accounting period found for unfixed sale date {sale_date}")
    if not period.can_modify_transactions():
        raise ValidationError(
            f"Cannot post unfixed sale dated {sale_date}: accounting period "
...
def _movement_payload(
    ...
    *,
    ...
    return CommodityMovementPayload(
        ...
        event_type=UNFIXED_SALE_MOVEMENT_EVENT_TYPE,
...
def _exposure_payload(
    ...
    *,
    ...
    return ExposurePayload(
        source=payload.source,
        event_type=UNFIXED_SALE_EXPOSURE_EVENT_TYPE,
        party=payload.party,
        commodity=payload.commodity,
...
def _valuation_currency(payload: UnfixedSalePostingPayload) -> str:
