...
✅ **Single Source of Truth**: Posting state is now derived from `Voucher.status` only
✅ **Simplified Database**: Removed redundant field from database schema
✅ **Reduced Migration**: Fewer database changes and migrations needed
✅ **Less Room for Sync Errors**: Can't have entry and voucher with mismatched statuses
✅ **Cleaner Architecture**: Status logic lives in one place (Voucher model)
...
**Changed:**
- Filter: `is_posted=True` → `voucher__status=VoucherStatus.POSTED`
...
✅ Check property returns correct values:
```python
...
✅ Posting engine creates entries:
