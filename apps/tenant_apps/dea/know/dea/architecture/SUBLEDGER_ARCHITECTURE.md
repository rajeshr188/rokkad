...
```
┌─────────────────────────────────────────────────────────┐
│             GENERAL LEDGER (Category Level)             │
│                                                          │
│  Question: "How much total money do customers owe us?" │
│  Answer: Check Accounts Receivable ledger = ₹50,000   │
│                                                          │
│  Ledger Model:                                          │
│  ┌──────────────────────────────────┐                  │
...
└─────────────────────────────────────────────────────────┘
            ...
            MUST RECONCILE (DR = CR)
                       ↕️
┌─────────────────────────────────────────────────────────┐
│              SUBLEDGER (Individual Level)               │
│                                                          │
│  Question: "Which customers owe us? How much each?"    │
│  Answer:                                                │
│  - ABC Corp: ₹30,000                                   │
│  - XYZ Ltd:  ₹20,000                                   │
│  ────────────────────                                   │
│  Total:      ₹50,000 ✅ (matches GL)                   │
│                                                          │
│  Account Model (per customer/vendor):                  │
│  ┌──────────────────────────────────┐                  │
│  │ Account: ABC Corp                │                  │
...
1. SalesInvoiceVoucher created (amount: ₹10,000 to ABC Corp)
        ↓
...
3. Creates JournalEntry with BOTH GL and Subledger lines:
   ...
   ┌────────────────────────────────────────┐
   │ ledgerno_dr = Accounts Receivable      │  (Asset ↑)
   │ ledgerno    = Sales Revenue            │  (Income ↑)
   │ amount      = ₹10,000                  │
   └────────────────────────────────────────┘

   AccountTransaction (Subledger):
   ┌────────────────────────────────────────┐
   │ Account     = ABC Corp (account record)│
   │ XactType    = Dr (Debit)               │
   │ amount      = ₹10,000                  │
   └────────────────────────────────────────┘
        ↓
4. AccountBalance view automatically updates:
   - ABC Corp balance = ₹30,000 + ₹10,000 = ₹40,000
```
...
### Method 2: Database View (`get_current_balance()`) ⭐ RECOMMENDED
...
if result['status'] == 'IMBALANCED':
    print(f"⚠️ ALERT: AR out of balance by {result['difference']}")
else:
    print(f"✅ AR reconciled: GL = Subledger = {result['gl_balance']}")
```
...
✅ **Performance**: Database view provides instant balance lookups  
✅ **Accuracy**: Two-sided journal entry design prevents imbalances  
✅ **Auditability**: AccountStatement snapshots create audit trail  
✅ **Scalability**: Supports multi-currency, millions of transactions  
✅ **Flexibility**: Both incremental and view-based calculations available  
