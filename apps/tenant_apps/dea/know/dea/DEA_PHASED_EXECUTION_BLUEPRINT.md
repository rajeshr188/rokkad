# DEA — Phased Execution Blueprint
**Branch:** `dea-kiss`  
**Authored:** 2026-05-06  
...
Each item carries:
- **File path** — exact file to create or change.
- **What** — the concrete action, not a description.
- **Why it matters** — the risk avoided or capability unlocked.
...
## Phase 1 — Correctness & Safety (days 1–30)
...
**What:**  
`generate_date_based_number` counts existing vouchers with `LIKE` pattern —
...
**File:** `apps/tenant_apps/dea/posting/engine.py` → `_write_journal_entry`
...
happens to be open at save time — incorrect when backfilling old dates.

**Acceptance:** Post a voucher dated in a closed prior period → `PostingError` raised.
Post a current-date voucher → JE has correct period FK.
...
Update `posting/engine.py` → `_write_journal_entry` to derive `PostingBundle` from
...
**Acceptance:** Attempt to insert two posted vouchers with the same fingerprint →
...
**Acceptance:** Close a period that has open loans with unposted accruals → accruals
...
**Acceptance:** Attempt period close with one DRAFT voucher in range → `ValidationError`
...
| VoucherLine → JE materialization | `python manage.py test dea.tests.test_voucher_line_posting` |
...
## Phase 2 — Accounting Completeness (days 31–60)
...
- `auto_match(bank_account, period)` — matches by date + amount with configurable
  tolerance.
- `get_unmatched_bank_lines(bank_account, period)` — unmatched = potential missing vouchers.
- `get_unmatched_gl_lines(ledger, period)` — GL entries not matched to any bank line.
...
`Account → SalesInvoiceVoucher → outstanding_amount, days_outstanding, bucket (0-30/31-60/61-90/90+)`.
...
`JournalEntry.posted_by` — no before/after state, no reason, no IP.

**Acceptance:** Post a voucher; verify `AccountingAuditEvent` row exists with
correct `payload_after`; attempt to delete the audit row → `PermissionDenied`.
...
`ApprovalRequest`: state machine `PENDING → APPROVED / REJECTED` with `approver`,
`reason`, `approved_at`.
...
**Acceptance:** Create approval policy for JVs > ₹1L; try to post such a JV →
...
- Finds templates due ≤ as_of_date.
...
A `Suspense` dashboard widget shows total suspense balance — anything non-zero
...
Phase 1 (correctness)
│
├── 1.1 Numbering         ← no dep
├── 1.2 Tenant context    ← no dep
├── 1.3 Period in engine  ← needs 1.2
├── 1.4 VoucherLine       ← no dep
├── 1.5 Fingerprint DB    ← no dep
├── 1.6 Remove print()    ← no dep
├── 1.7 Period nav guard  ← no dep
├── 1.8 Accrual wiring    ← needs 1.3
└── 1.9 Draft guard       ← no dep

Phase 2 (completeness)
│
├── 2.1 Pre-close checklist  ← needs 1.8, 1.9
├── 2.2 Depreciation         ← needs 1.4 (VoucherLine)
├── 2.3 Prepaid              ← needs 1.4
├── 2.4 Bank reconciliation  ← needs 1.4
├── 2.5 Financial reports    ← needs 1.3, 2.2
├── 2.6 VoucherLine formset  ← needs 1.4
└── 2.7 AR/AP settlement     ← needs 1.4

Phase 3 (hardening)
│
├── 3.1 Audit log            ← needs Phase 2 complete
├── 3.2 Approval matrix      ← needs 3.1
├── 3.3 RBAC                 ← needs 3.1
├── 3.4 Revaluation          ← needs 2.5
├── 3.5 Recurring vouchers   ← needs 2.6
├── 3.6 Balance snapshots    ← needs Phase 2 complete
└── 3.7 Suspense             ← needs 3.1
```
