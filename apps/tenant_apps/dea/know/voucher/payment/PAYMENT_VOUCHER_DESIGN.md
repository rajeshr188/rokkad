...
```
┌──────────────────────────────────────────────────────────────────┐
│ BUSINESS EVENTS (Contracts/Agreements)                           │
│ ─────────────────────────────────────────────────────────────────│
│                                                                   │
│  GivenLoan created → Records agreement to lend                   │
│  Status: CREATED → APPROVED → DISBURSED (just milestones)       │
│  NO ACCOUNTING POSTING at creation!                              │
│                                                                   │
│  TakenLoan created → Records agreement to borrow                 │
│  NO ACCOUNTING POSTING at creation!                              │
│                                                                   │
└──────────────────────────────────────────────────────────────────┘
                              ↓
                 ...
                 When does cash actually move?
                              ↓
┌──────────────────────────────────────────────────────────────────┐
│ ECONOMIC EVENTS (Cash Movements)                                 │
│ ─────────────────────────────────────────────────────────────────│
│                                                                   │
│  PaymentVoucher created → Records ACTUAL cash movement           │
│  payment_type: DISBURSAL | RECEIPT | OTHER                      │
│  direction: PAYMENT (cash out) | RECEIPT (cash in)              │
│  payment_date: When cash actually moves (not contract date)      │
│                                                                   │
│  AUTO POSTING ON SAVE → Creates Voucher + JournalEntries       │
...
└──────────────────────────────────────────────────────────────────┘
        ...
        │ GIVENLOAN_DISBURSAL                       │
        │ GIVENLOAN_RECEIPT                        │
        │ TAKENLOAN_RECEIPT (we receive)           │
        │ TAKENLOAN_PAYMENT (we pay back)          │
        │ SALES_RECEIPT                            │
        │ PURCHASE_PAYMENT                         │
        └──────────────────────────────────────────┘
                              ↓
                    PostingBundle → JournalEntry
```
...
```
GivenLoan creation → Immediate posting
                  → Dr LOAN_RECEIVABLE ₹20,000
                  → Cr CASH ₹20,000
...
```
GivenLoan creation → No posting (just business record)

Day 1: PaymentVoucher (DISBURSAL) → Posting at actual cash transfer
       Dr LOAN_RECEIVABLE ₹20,000
       Cr CASH ₹20,000  ← Cash actually decreases here

Day 15: PaymentVoucher (RECEIPT) → Posting at actual cash receipt
        Dr CASH ₹10,000  ← Cash actually increases here
        Cr LOAN_RECEIVABLE ₹10,000
```
...
```
Loan creation → Immediate accounting posting
              → Overstates cash outflow  
              → Posts before cash moves
              → Exchange rate locks at creation
              → Violates IFRS 9
...
```
Loan creation → Just business record (NO posting)
              
PaymentVoucher → ONLY created when cash moves
              → Accurate cash flow reporting
              → Exchange rate at actual payment
              → IFRS 9 compliant
              → Natural handling of partial/multiple payments
```
...
class PaymentVoucher(BusinessDoc):
    ...
    def __str__(self):
        direction_symbol = "←" if self.direction == CashFlowDirection.RECEIPT else "→"
...
# ✅ IMPORTANT: NO automatic posting here!
...
# ✅ NOW posting happens automatically!
# Voucher: GIVENLOAN_DISBURSAL
# Entry: Dr LOAN_RECEIVABLE ₹20,000, Cr CASH ₹20,000


# Day 15: Customer pays ₹10,000
...
# Entry: Dr CASH ₹10,000, Cr LOAN_RECEIVABLE ₹10,000
```
...
class GivenLoan(BaseLoan, ...):
    def save(self, *args, **kwargs):
        ...
        # ✅ NO POSTING HERE - Leave it to PaymentVoucher
        # GivenLoan is just a business record, not an economic event
```

**Why:**
- GivenLoan creation ≠ cash outflow
...
class GivenLoan(BaseLoan, GivenLoanReleaseMixin):
    ...
    def save(self, *args, **kwargs):
        ...
        # ❌ REMOVE automatic posting
        # if self.auto_post_to_accounting:
        #     self._auto_post_to_accounting()
        
        # ✅ Posting now happens in PaymentVoucher.save() instead
```
...
**Impact**: High
- Need data migration from LoanPayment → PaymentVoucher
...
# Money object
print(payment.total_amount)  # ₹100.00
...
# Arithmetic
new_total = payment.total_amount + Money(50, 'INR')  # ₹150.00
...
{% if payment.is_multicurrency %}
    ...
    <span class="base-amount">≈ {{ payment.amount_in_base_currency }}</span>
{% else %}
...
- [ ] Create payment → auto-posts to accounting
...
- [ ] Bidirectional access (loan→payments, payment→loan)
...
1. **Data Migration**
   - Risk: Data loss or corruption during LoanPayment → PaymentVoucher migration
   - Mitigation: 
