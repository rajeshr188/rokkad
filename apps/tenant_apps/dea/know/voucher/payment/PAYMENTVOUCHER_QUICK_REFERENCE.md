...
### List & Detail
- `/dea/payments/` → Payment list with filters
- `/dea/payments/<id>/` → Payment detail view

### Create & Modify
- `/dea/payments/create/` → Create new payment
- `/dea/payments/<id>/edit/` → Edit draft payment
- `/dea/payments/<id>/delete/` → Delete draft payment

### Loan Payment (AJAX)
- `/dea/loans/<type>/<id>/payment/create/` → Quick payment from loan
...
All rules:
- ✅ Automatically triggered on payment creation
- ✅ Create appropriate journal entries
- ✅ Update GL and subledger balances
...
# Dr CASH ₹5,000
# Cr LOAN_RECEIVABLE ₹5,000
# Customer subledger updated (Cr side)

# Check updated balances
print(loan.total_received)  # ₹5,000
print(loan.outstanding_principal)  # Initial - ₹5,000
```
...
# Dr LOAN_RECEIVABLE ₹100,000
# Cr CASH ₹100,000
# Customer subledger: Dr ₹100,000 (they owe us)
```
...
print(payment.amount_in_base_currency)  # ₹12,525.00
...
print(payment.principal_amount)  # ₹5,000
print(payment.interest_amount)  # ₹500
```
...
### ✅ DO
- ✅ Create payments ONLY when cash actually moves
- ✅ Use meaningful reference numbers for traceability
- ✅ Set `is_final_payment=True` when loan closes
- ✅ Use appropriate payment methods for records
- ✅ Add descriptions for complex payments
- ✅ Create in drafts first if you want to review
- ✅ Use Money objects for amounts (supports currency)
- ✅ Check `payment.posted` before editing

### ❌ DON'T
- ❌ Don't create payments at contract creation (wait for cash)
- ❌ Don't edit payments after they're posted (create reversal instead)
- ❌ Don't mix up principal/interest allocation
- ❌ Don't mix up direction (RECEIPT vs PAYMENT confusing)
- ❌ Don't leave reference_number blank for bank transfers
- ❌ Don't ignore validation errors

### 📝 Notes on Payment Direction
