...
```
┌─────────────────────────────────────────────────────────────────┐
│                  SIX ESSENTIAL VOUCHERS (MVP ERP)                │
├──────────────────────┬──────────────────────┬───────────────────┤
│     PRIORITY 1       │     PRIORITY 2       │    PRIORITY 3     │
│    (Week 1-2)        │    (Week 3-4)        │     (Later)       │
├──────────────────────┼──────────────────────┼───────────────────┤
│                                                                  │
│ 1. PAYMENT VOUCHER   │ 4. SALES INVOICE     │ 6. STOCK MOVEMENT │
│    ✓ Cash flows      │    □ Revenue         │    □ Inventory    │
│    ✓ Receipts/       │    □ AR tracking     │    □ Movement     │
│      Payments        │                      │      tracking      │
│                      │ 5. PURCHASE INVOICE  │                    │
│ 2. EXPENSE VOUCHER   │    □ Inventory/      │ 7. DEPRECIATION   │
│    ✓ Company costs   │      Expense         │    □ Period-end   │
│    ✓ Accruals        │    □ AP tracking     │      adjustment    │
│    ✓ Tax handling    │                      │    □ Asset value   │
│                      │                      │                    │
│ 3. JOURNAL ENTRY     │                      │                    │
│    ✓ Manual GL       │                      │                    │
│      adjustments     │                      │                    │
...
├─ Creates SOURCE DOCUMENT (Invoice, Expense Claim, etc.)
├─ Source Doc approved → Creates VOUCHER
├─ Voucher posted → Creates JOURNAL ENTRY #1
│  └─ Records: Asset/Expense ↔ Liability/Revenue
│
└─ Creates PAYABLE/RECEIVABLE (not yet settled)

         ↓ TIME PASSES (days/weeks)

CASH EVENT (Economic Transaction)
├─ Creates PAYMENT VOUCHER
├─ Payment Voucher posted → Creates JOURNAL ENTRY #2
│  └─ Records: Cash ↔ Payable/Receivable
│
...
**Example Timeline:**
```
Day 1: Purchase goods → PurchaseInvoice → Voucher → JE (DR Inventory, CR AP)
Day 15: Pay supplier → PaymentVoucher → JE (DR AP, CR Cash)

Result:
- Day 1-14: AP shows liability, Cash unchanged ✅ Accurate
- Day 15: AP reduced, Cash decreased ✅ Accurate
```
...
class ExpenseVoucher(BusinessDoc):
    ...
    - Posts to GL immediately (Expense ↔ Payable)
    - Cash settlement handled separately by PaymentVoucher
    - Supports complex tax scenarios (GST, TDS)
    
    Examples:
    - Employee expense claim → Approved → ExpenseVoucher → PaymentVoucher (reimbursement)
    - Vendor bill for services → Received → ExpenseVoucher → PaymentVoucher (payment)
    - Rent accrual → Month-end → ExpenseVoucher (no payment yet)
    """
...
class EmployeeExpenseClaimRule(BasePostingRule):
    ...
    Example:
    Employee submits ₹10,000 claim (Travel ₹6,000 + Food ₹4,000)
    DR: Travel Expense ₹6,000
    DR: Food Expense ₹4,000
    CR: Employee Payable ₹10,000
    """
...
class VendorBillExpenseRule(BasePostingRule):
    ...
    Example:
    Professional service bill for ₹50,000 + GST ₹9,000 - TDS ₹5,000
    DR: Professional Services Expense ₹50,000
    DR: GST Input Credit ₹9,000
    CR: TDS Payable ₹5,000
    CR: Vendor Payable ₹54,000
    """
...
1. Employee submits expense claim
   ...
   ├─ Status: SUBMITTED
   ├─ Line items: Travel, Food, etc.
   └─ Receipts attached

2. Manager reviews & approves
   ├─ ExpenseClaim.status = APPROVED
   └─ Triggers: Create ExpenseVoucher
      ├─ source_document = ExpenseClaim
      ├─ source_type = EMP_CLAIM
      ├─ party = Employee
      └─ net_payable = calculated
...
4. Accountant creates payment
   ├─ PaymentVoucher created
   ├─ source_document = ExpenseVoucher
   ├─ direction = PAYMENT
   ├─ amount = net_payable
   └─ Posts:
      ├─ DR: Employee Payable
...
5. Employee receives money
   ├─ ExpenseVoucher.paid_amount updated
   ├─ ExpenseVoucher.is_paid = True
   └─ Workflow complete
...
1. Customer places order (optional)
   ├─ SalesOrder created
   ├─ Status: PENDING
   └─ Items: Product A x 10, Product B x 5

2. Goods delivered to customer
   ├─ Delivery Note created
   └─ Triggers: Create SalesInvoiceVoucher
...
4. SalesInvoiceVoucher posts
   ...
   │  ├─ CR: Sales Revenue ₹100,000
   │  └─ CR: GST Output ₹18,000
   └─ Creates AR (receivable from customer)

5. Customer pays (later)
   ├─ PaymentVoucher created
   ├─ source_document = SalesInvoiceVoucher
   ├─ direction = RECEIPT
   ├─ amount = ₹118,000
   └─ Posts:
      ├─ DR: Cash ₹118,000
...
6. Payment received
   ├─ SalesInvoiceVoucher.received_amount updated
   ├─ SalesInvoiceVoucher.is_fully_paid = True
   └─ AR cleared
```
...
class PurchaseGoodsRule(BasePostingRule):
    ...
    Example:
    Purchased goods for ₹100,000 + GST ₹18,000
...
2. Goods received
   ├─ GoodsReceiptNote created
   ├─ Quantity verified
   ├─ Quality checked
   └─ Status: RECEIVED

3. Vendor invoice received
   ├─ PurchaseInvoiceVoucher created
   ├─ invoice_number: From vendor
...
5. PurchaseInvoiceVoucher posts
   ...
   │  ├─ DR: GST Input ₹18,000
   │  └─ CR: AP ₹118,000
   └─ Creates AP (payable to vendor)

6. Pay vendor (later)
   ├─ PaymentVoucher created
   ├─ source_document = PurchaseInvoiceVoucher
   ├─ direction = PAYMENT
   ├─ amount = ₹118,000
   └─ Posts:
      ├─ DR: AP ₹118,000
...
7. Vendor receives payment
   ├─ PurchaseInvoiceVoucher.paid_amount updated
   ├─ PurchaseInvoiceVoucher.is_fully_paid = True
   └─ AP cleared
```
...
1. Accountant creates manual journal entry
   ├─ JournalEntryVoucher created
   ├─ entry_type: CLOSING, ACCRUAL, CORRECTION, etc.
   ├─ description: Detailed explanation
   └─ Status: DRAFT

2. Add line items
   ├─ For each debit and credit:
   │  ├─ Select GL account (Ledger)
   │  ├─ Enter amount (always positive)
   │  ├─ Select side (DR or CR)
...
3. Save journal entry
   ├─ JournalEntryVoucher.total_debit calculated
   ├─ JournalEntryVoucher.total_credit calculated
   └─ Status: DRAFT

4. Manager reviews and approves
   ├─ JournalEntryVoucher.reviewed_by = Manager
   ├─ JournalEntryVoucher.reviewed_at = now
   ├─ Status: APPROVED
   └─ Triggers: Post to GL

5. Auto-post on approval
   ├─ JournalEntry created
   │  ├─ Posted by: Posting Engine
   │  ├─ Posted at: now
   │  └─ Period: Auto-detected from date
   │
   ├─ Ledger entries created (per line item)
   │  ├─ DR entries → Ledger balances ↑
   │  └─ CR entries → Ledger balances ↓
   │
   └─ GL immediately updated

Example: Depreciation Accrual
┌──────────────────────────────────────┐
│ JournalEntryVoucher                  │
├──────────────────────────────────────┤
│ JE Number: JE-2024-02-0001           │
│ Entry Type: ACCRUAL                  │
│ Total DR: ₹5,000 = Total CR: ₹5,000 │
│                                      │
│ Line Items:                          │
│ 1. DR: Depreciation Exp. ₹5,000     │
...
1. Goods received into warehouse
   ├─ Vendor sends goods
   └─ Triggers: Create StockMovementVoucher
      ├─ reason: RECEIPT
      ├─ to_location: Warehouse A
      ├─ reference: PO-xxxxx
      └─ line_items: Product A x 100 @ ₹100

2. Record movement
   ├─ StockMovementVoucher created
   ├─ Status: DRAFT
   └─ Validate:
      ├─ Product exists ✓
      ├─ Location exists ✓
...
3. Approve movement
   ├─ Status: APPROVED
   └─ System updates:
      ├─ Inventory balance ↑ in Warehouse A
      └─ (No GL posting for receipts)
...
4. Transfer goods between locations
   ...
   └─ line_items: Product A x 50

5. Approve transfer
   ├─ Status: APPROVED
   └─ System updates:
      ├─ Warehouse A balance ↓ 50 units
      ├─ Production Floor balance ↑ 50 units
      └─ (No GL posting for transfers)

6. Stock damage/loss
   ├─ StockMovementVoucher created
   ├─ reason: DAMAGED (or LOSS)
   ├─ from_location: Warehouse A
   └─ line_items: Product B x 10 @ loss value
...
class DepreciationRule(BasePostingRule):
    ...
    Example:
    Asset: Equipment @ ₹100,000, 5-year life
    Monthly depreciation: ₹1,667
...
1. Month-end approaching
   ├─ Accountant prepares depreciation
   └─ Trigger: Create DepreciationVoucher
      ├─ period_start: Feb 1, 2024
      ├─ period_end: Feb 29, 2024
      └─ description: "February 2024 Depreciation"

2. Calculate asset depreciation
   ├─ For each fixed asset:
   │  ├─ Get: Original Cost, Useful Life, Salvage Value
   │  ├─ Calculate: Monthly Depreciation
   │  ├─ Get: Accumulated Before
   │  └─ Create DepreciationLineItem
   │
   └─ Add all lines to voucher
...
├─ Monthly: ₹1,500
├─ Accumulated Before: ₹7,500 (5 months)
├─ This Month: ₹1,500
├─ Accumulated After: ₹9,000
└─ Remaining Value: ₹91,000

3. Voucher created
   ├─ Total Depreciation: ₹1,500 × N assets
   ├─ Status: DRAFT
   └─ Lines: One per asset

4. Review and approve
   ├─ Accountant reviews calculations
   ├─ Status: APPROVED
   └─ Triggers: Post to GL
...
6. GL Updated
   ├─ Depreciation Expense ↑ ₹1,500 (P&L)
   ├─ Equipment value on BS:
   │  ├─ Original: ₹100,000
   │  ├─ Less: Accumulated Dep ₹9,000
   │  └─ Net Book: ₹91,000
   └─ Monthly workflow complete
```
...
### Complete Flow: Expense → Payment

```
┌──────────────────────────────────────────────────────────┐
│ EMPLOYEE EXPENSE CLAIM → PAYMENT COMPLETE FLOW           │
├──────────────────────────────────────────────────────────┤
│                                                           │
│ DAY 1: Employee submits claim                            │
│ ├─ ExpenseClaim created (source document)               │
│ ├─ Line items: Travel, Food, Professional               │
│ ├─ Total: ₹15,500                                        │
│ └─ Status: SUBMITTED                                     │
│                                                           │
│ DAY 2: Manager approves                                  │
│ ├─ ExpenseClaim.status = APPROVED                        │
│ └─ Signal triggers: Create ExpenseVoucher               │
│    ├─ source_document = ExpenseClaim                     │
│    ├─ source_type = EMP_CLAIM                           │
│    ├─ party = Employee                                   │
│    ├─ gross_amount = ₹15,500                            │
│    ├─ tax_amount = ₹900 (GST on professional)          │
│    ├─ tds_amount = ₹500 (TDS on professional)          │
│    ├─ net_payable = ₹15,900                            │
│    └─ Status: DRAFT                                      │
│                                                           │
│ DAY 2: ExpenseVoucher auto-posts                         │
│ ├─ Status: DRAFT → POSTED                               │
│ ├─ Posting engine creates JournalEntry #1:              │
│ │  ├─ DR: Travel Expense ₹2,500                         │
│ │  ├─ DR: Food Expense ₹1,200                           │
│ │  ├─ DR: Professional Services ₹5,000                  │
│ │  ├─ DR: GST Input Credit ₹900                         │
│ │  ├─ DR: Office Supplies ₹3,800                        │
│ │  ├─ DR: Employee Reimb. ₹3,000                        │
│ │  ├─ CR: TDS Payable ₹500                              │
│ │  └─ CR: Employee Payable ₹15,900                      │
│ │                                                         │
│ └─ GL balances updated:                                  │
│    ├─ Expense accounts ↑                                 │
│    └─ Employee Payable ↑ (liability created)             │
│                                                           │
│ DAY 3: Accountant creates payment                        │
│ ├─ PaymentVoucher created                                │
│ │  ├─ source_document = ExpenseVoucher                   │
│ │  ├─ direction = PAYMENT (cash OUT)                     │
│ │  ├─ payment_type = REIMBURSEMENT                       │
│ │  ├─ total_amount = ₹15,400 (net of TDS)              │
│ │  └─ payment_method = BANK                              │
│ │                                                         │
│ ├─ PaymentVoucher auto-posts                             │
│ ├─ Creates JournalEntry #2:                              │
│ │  ├─ DR: Employee Payable ₹15,900                      │
│ │  ├─ CR: TDS Payable ₹500 (reversed from accrual)     │
│ │  └─ CR: Cash/Bank ₹15,400                             │
│ │                                                         │
│ └─ GL balances updated:                                  │
│    ├─ Employee Payable ↓ (liability cleared)             │
│    └─ Cash ↓ (money went out)                            │
│                                                           │
│ DAY 3: ExpenseVoucher updated                            │
│ ├─ paid_amount = ₹15,400                                │
│ ├─ is_paid = True                                        │
│ └─ Workflow complete ✅                                  │
│                                                           │
...
### Complete Flow: Sales → Collection

```
┌──────────────────────────────────────────────────────────┐
│ SALES INVOICE → PAYMENT COLLECTION COMPLETE FLOW         │
├──────────────────────────────────────────────────────────┤
│                                                           │
│ DAY 1: Customer places order                             │
│ ├─ SalesOrder created                                    │
│ ├─ Customer: XYZ Corp                                    │
│ ├─ Items: Product A x 10                                 │
│ └─ Status: CONFIRMED                                     │
│                                                           │
...
│    └─ Status: DRAFT                                      │
│                                                           │
│ DAY 2: SalesInvoiceVoucher approved & posted             │
│ ├─ Status: DRAFT → POSTED                               │
│ ├─ Creates JournalEntry #1:                              │
│ │  ├─ DR: Accounts Receivable ₹118,000                  │
│ │  ├─ CR: Sales Revenue ₹100,000                        │
│ │  ├─ CR: CGST Output ₹9,000                            │
│ │  └─ CR: SGST Output ₹9,000                            │
│ │                                                         │
│ └─ GL balances updated:                                  │
│    ├─ AR (asset) ↑ ₹118,000                             │
│    ├─ Sales Revenue ↑ ₹100,000                          │
│    └─ GST Output (liability) ↑ ₹18,000                  │
│                                                           │
...
│ │                                                         │
│ └─ GL balances updated:                                  │
│    ├─ Cash ↑ ₹118,000 (money received)                  │
│    └─ AR ↓ ₹118,000 (receivable cleared)                │
│                                                           │
│ DAY 15: SalesInvoiceVoucher updated                      │
│ ├─ received_amount = ₹118,000                           │
│ ├─ is_fully_paid = True                                 │
│ └─ Workflow complete ✅                                  │
│                                                           │
└──────────────────────────────────────────────────────────┘
...
### Complete Flow: Purchase → Payment

```
┌──────────────────────────────────────────────────────────┐
│ PURCHASE INVOICE → VENDOR PAYMENT COMPLETE FLOW          │
├──────────────────────────────────────────────────────────┤
│                                                           │
│ DAY 1: Purchase order placed                             │
│ ├─ PurchaseOrder created                                 │
│ ├─ Vendor: ABC Supplies                                  │
│ ├─ Items: Raw material x 100                             │
│ └─ Status: APPROVED                                      │
│                                                           │
│ DAY 5: Goods received                                    │
│ ├─ GoodsReceiptNote created                              │
│ ├─ Quantity verified: 100 units ✓                        │
│ ├─ Quality checked ✓                                     │
│ └─ Status: RECEIVED                                      │
│                                                           │
│ DAY 6: Vendor invoice received                           │
│ ├─ PurchaseInvoiceVoucher created                        │
│ │  ├─ invoice_number: VENDOR-INV-001                     │
│ │  ├─ internal_number: PI-2024-02-0001                   │
│ │  ├─ vendor: ABC Supplies                               │
│ │  ├─ subtotal: ₹100,000                                │
│ │  ├─ gst: ₹18,000                                       │
│ │  ├─ total: ₹118,000                                   │
...
│                                                           │
│ DAY 6: PurchaseInvoiceVoucher approved & posted          │
│ ├─ Status: DRAFT → POSTED                               │
│ ├─ Creates JournalEntry #1:                              │
│ │  ├─ DR: Inventory ₹100,000                            │
│ │  ├─ DR: GST Input Credit ₹18,000                      │
│ │  └─ CR: Accounts Payable ₹118,000                     │
│ │                                                         │
│ └─ GL balances updated:                                  │
│    ├─ Inventory (asset) ↑ ₹100,000                      │
│    ├─ GST Input (asset) ↑ ₹18,000                       │
│    └─ AP (liability) ↑ ₹118,000                         │
│                                                           │
│ DAY 20: Pay vendor (as per credit terms)                 │
│ ├─ PaymentVoucher created                                │
│ │  ├─ source_document = PurchaseInvoiceVoucher           │
│ │  ├─ direction = PAYMENT (cash OUT)                     │
...
│ │  └─ CR: Cash/Bank ₹118,000                            │
│ │                                                         │
│ └─ GL balances updated:                                  │
│    ├─ AP ↓ ₹118,000 (liability cleared)                 │
│    └─ Cash ↓ ₹118,000 (money paid)                      │
│                                                           │
│ DAY 20: PurchaseInvoiceVoucher updated                   │
│ ├─ paid_amount = ₹118,000                               │
│ ├─ is_fully_paid = True                                 │
│ └─ Workflow complete ✅                                  │
│                                                           │
...
```
WEEK 1-2:  ✓ Payment + Expense + Journal Entry models & rules
WEEK 2-3:  ✓ Posting engine & auto-creation signals
WEEK 3-4:  ✓ Priority 1 views, forms, templates
WEEK 4:    ✓ Sales Invoice Voucher complete
WEEK 5:    ✓ Purchase Invoice Voucher complete
WEEK 5-6:  ✓ Payment integration & reports
WEEK 6-7:  ✓ Stock Movement Voucher complete
WEEK 7-8:  ✓ Depreciation Voucher complete
...
- ✅ All 6 voucher types fully implemented
- ✅ Django models with complete field coverage
- ✅ All posting rules working correctly
- ✅ GL balances reconcile with source documents
...
| **Critical for MVP** | ✅ Yes | ✅ Yes | ✅ Yes | ✅ Yes | ✅ Yes | ⏳ Phase 2 | ⏳ Phase 2 |
...
├─ Debit Accounts: User selects (any GL account)
├─ Credit Accounts: User selects (any GL account)

STOCK MOVEMENT VOUCHER:
├─ Debit Accounts:
│  ├─ Loss/Damage Expense (if damage/loss)
│  └─ N/A (if transfer)
├─ Credit Accounts:
│  ├─ Inventory (if damage/loss)
│  └─ N/A (if transfer)
...
**✅ PRIORITY 1 - Core Transactional (Week 1-4):**
...
**✅ PRIORITY 2 - Sales & Purchase (Week 4-6):**
...
**✅ PRIORITY 3 - Inventory & Assets (Week 6-8):**
- Stock Movement Voucher - Inventory transfers & adjustments
- Depreciation Voucher - Fixed asset depreciation scheduling

**Included in This Document:**
- ✅ Complete Django models (all 7 voucher types)
- ✅ Posting rules for all scenarios (15+ rule implementations)
- ✅ Line item models with tax & cost handling
- ✅ Complete end-to-end workflows (3 detailed examples)
- ✅ GL account mapping and posting logic
...
- ✅ Comparison matrix for all voucher types
...
2. ⏳ Create models and migrations (Week 1)
3. ⏳ Implement posting rules (Week 2-3)
4. ⏳ Build views and forms (Week 3-4)
5. ⏳ Create templates and workflows (Week 4-5)
6. ⏳ Test end-to-end scenarios (Week 6)
7. ⏳ Deploy to production (Week 7-8)

---

**Document Version**: 1.0  
**Last Updated**: February 26, 2024  
**Status**: ✅ Complete - All 6 Voucher Types Fully Designed  
