...
4. Refactor existing helpers to delegate to the new service:
   - `record_loan_release` → `GivenLoanPostingService.post_release`
   - `record_loan_auction` → `GivenLoanPostingService.post_auction_recovery`
   - `record_loan_sale` → `GivenLoanPostingService.post_sale_recovery`
   - repayment flow in `loanpayment.py` → `GivenLoanPostingService.post_repayment`
...
7. Add a compatibility façade in `apps/tenant_apps/girvi/service_modules/__init__.py` if necessary,
...
views/loanpayment.py
    └─> GivenLoanPostingService.post_repayment()

release_lifecycle.py
    └─> payment.record_loan_release()
           └─> loan_posting.GivenLoanPostingService.post_release()
...
payment.py
    └─> wrapper façade around loan_posting service
```
...
## DB architecture: GivenLoan → DEA accounting documents
...
4. Add compatibility exports:
   ...
   - Export `GivenLoanPostingService` and the existing `payment` façade symbols used by legacy imports.
