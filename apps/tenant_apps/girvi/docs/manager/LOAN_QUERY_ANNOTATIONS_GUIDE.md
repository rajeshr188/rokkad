...
- ✅ Clearer performance requirements
- ✅ Easy to modify one calculation without affecting others
...
for loan in loans:
    print(f"Collateral value: ₹{loan.total_current_value}")
```
...
<table>
    ...
    <tbody>
        ...
        <tr class="{% if loan.is_overdue %}table-danger{% endif %}">
            ...
            <td>₹{{ loan.gold_loanamount|floatformat:2 }}</td>
            <td>{{ loan.silver_weight|floatformat:3 }}</td>
            <td>₹{{ loan.total_interest|floatformat:2 }}</td>
            <td>₹{{ loan.total_due|floatformat:2 }}</td>
            <td>₹{{ loan.total_current_value|floatformat:2 }}</td>
            <td>
                {% if loan.is_overdue %}
...
<div class="dashboard">
    ...
    <div class="card">
        ...
        <div class="metrics">
            ...
            <div>
                <strong>Total Due:</strong> ₹{{ non_performing.total_due|floatformat:2 }}
            </div>
            <div>
                <strong>Total Collateral Value:</strong> 
                ₹{{ non_performing.total_collateral_value|floatformat:2 }}
            </div>
            <div>
                <strong>Difference (under-collateralized by):</strong>
                ₹{{ non_performing.total_due|add:non_performing.total_collateral_value|floatformat:2 }}
            </div>
        </div>
...
print(f"Shortfall: ₹{stats['total_due'] - stats['avg_collateral_value']}")
```
