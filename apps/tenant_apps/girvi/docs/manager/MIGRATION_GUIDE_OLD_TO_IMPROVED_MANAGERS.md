# Migration Guide: Old Managers → Improved Managers
...
|-----------|-----------|--------|
| `with_details(grate, srate, brate)` | `for_table_display()` | ✅ Replaced |
| `with_total_interest()` | `with_interest_metrics()` | ✅ Replaced |
| `_get_rates()` | `RateCacheService` | ✅ Replaced |
| `months_since_or_to_release()` | `with_duration_metrics()` | ✅ Replaced |
| `with_itemwise_loanamount()` | `with_itemwise_amounts()` | ✅ Replaced |
| `total_itemwise_loanamount()` | `with_itemwise_amounts() + aggregate()` | ✅ Replaced |
| `total_current_value()` | `with_current_value() + aggregate()` | ✅ Replaced |
| `total_weight()` | `with_metal_weights() + aggregate()` | ✅ Replaced |
| `total_pure_weight()` | `with_metal_weights() + aggregate()` | ✅ Replaced |
| `itemwise_value()` | `with_current_value() + aggregate()` | ✅ Replaced |
| `total_loanamount()` | `aggregate(total=Sum(...))` | ✅ Replaced |

### Manager Methods

| Old | New | Status |
|-----|-----|--------|
| Manual dashboard queries | `non_performing_loans_stats()` | ✅ New |
| Manual dashboard queries | `long_dead_loans_stats()` | ✅ New |
| N/A | `overdue()` | ✅ New convenience |
| N/A | `good_standing()` | ✅ New convenience |
| N/A | `long_dead()` | ✅ New convenience |
...
├── manager_improved.py  ← Use this
├── managers.py          ← Keep as reference/backup
└── models.py            ← Update to use ImprovedLoanManager
```
...
```
apps/tenant_apps/girvi/
├── manager_improved.py  ← New
├── managers.py          ← Old (still used)
└── models.py            ← Both available
```
...
### Week 1: Setup
- [ ] Create `manager_improved.py` ✅ Done
