# Series Guardrails - Implementation Complete ✓
...
✅ **Loan Count Thresholds** - Limit max active loans per series  
✅ **Loan Amount Thresholds** - Limit max total amount per series  
✅ **Smart Deactivation** - LOANS_ONLY, RELEASES_ONLY, or BOTH operations  
✅ **Automatic Enforcement** - Signals auto-trigger on loan operations  
✅ **Form Integration** - Deactivated series hidden from loan/release forms  
✅ **Admin Dashboard** - Visual status indicators and controls  
✅ **Tracking** - Records when and why deactivation occurred  
...
1. Series Created with Thresholds
   ├─ loan_count_threshold: 100
   ├─ loan_amount_threshold: ₹5M
   └─ deactivation_rule: LOANS_ONLY

2. Loan Created (Signal Triggered)
   └─ Calls: series.check_and_apply_deactivation()

3. Threshold Check
   ├─ Current loans: 100 >= threshold 100? YES
   └─ Current amount: ₹5M >= threshold ₹5M? YES

4. Apply Rule (LOANS_ONLY)
   └─ Set: deactivated_for_loans = True

5. Form Uses Manager
   └─ Series.objects.active_for_loans() excludes deactivated series

6. Result
   └─ User cannot see this series in loan form
```
...
- ⚠️ Threshold exceeded - Limit approaching/reached
- 🔒 Loans disabled - Cannot create loans
- 🔒 Releases disabled - Cannot create releases
- 🔒 Locked (both) - Completely disabled
...
```
Loan Form
├─ Series: [GOLD ▼] ← All active series shown
├─ Series: [SILVER ▼] ← Even at capacity
└─ Series: [PLATINUM ▼] ← No warnings
```
...
Loan Form
├─ Series: [GOLD ▼]
│  └─ GOLD at 100 loans (limit: 100)
│     → NOT SHOWN IN DROPDOWN
└─ Series: [SILVER ▼]
   └─ Can still create loans in SILVER
```
...
gold.loan_amount_threshold = Decimal("50000000")  # Max ₹50M
...
```
Signal fires → checks threshold → exceeds limit
→ sets deactivated_for_loans = True
→ records deactivation_date = now()
→ records reason = "Loan count (500) reached threshold (500)"
```
...
1. Opens Loan Form
2. Series dropdown shows: GOLD (✓) [still listed if active]
   But actually checks can_create_loan()
3. If deactivated → GOLD doesn't appear
...
print(f"Current amount: ₹{status['current_loan_amount']}/50M")
...
→ Check `deactivated_for_loans` flag and `can_create_loan()` method

**Loans creating in deactivated series?**
→ Check if form is using `active_for_loans()` manager

**Deactivation not triggering?**
→ Check signal handler logs and deactivation rule setting
...
**Setup Complete!** You can now control series capacity automatically. 🎉
