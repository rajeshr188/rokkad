...
def record_loan_release(release, created_by):
    ...
    outstanding = loan.outstanding_principal  # Money(remaining_balance, "INR")
    principal_val = outstanding.amount if hasattr(outstanding, "amount") else Decimal(str(outstanding))

    interest_raw = loan.interest_due()  # Decimal: total accrued interest
...
def reverse_loan_disbursal(loan: GivenLoan, user):
    ...
    # Block if any other posted vouchers exist (repayments, release, etc.)
    other_posted = (
