...
from apps.tenant_apps.girvi.models import Loan, Statement, StatementItem  # Adjust the import as needed
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Tenant with name {tenant} does not exist."))
            return

        with schema_context(tenant.name):
            # Create a new Statement instance
            statement = Statement.objects.create(created_by=None)  # Adjust created_by as needed
                    ...
                    try:
                        ...
                        if not loan.is_released:
                            try:
                                ...
                                self.stdout.write(self.style.ERROR(f"Duplicate entry for loan ID {loan_id} in statement {statement.id}."))
                        else:
                            try:
                                StatementItem.objects.create(
                                    statement=statement,
                        ...
                        self.stdout.write(self.style.ERROR(f"Loan with ID {loan_id} does not exist."))

            self.stdout.write(self.style.SUCCESS("Statement and StatementItems created successfully."))