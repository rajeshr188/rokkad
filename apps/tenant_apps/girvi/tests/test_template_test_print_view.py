...
from apps.tenant_apps.girvi.views.template import template_test_print


class TemplateTestPrintViewTests(SimpleTestCase):
    def setUp(self):
    ...
    def test_template_test_print_redirects_when_no_sample_loan_exists(
        self,
        mock_require_admin,
        mock_givenloan,
        ...
        request = self.factory.get("/girvi/templates/7/test-print/")
        request.user = self.user

        response = template_test_print.__wrapped__(request, pk=7)
    ...
    @patch("apps.tenant_apps.girvi.views.template.GivenLoan")
    @patch("apps.tenant_apps.girvi.views.template._require_template_admin")
    def test_template_test_print_returns_pdf_for_sample_loan(
        self,
        mock_require_admin,
        mock_givenloan,
        ...
        loan = SimpleNamespace(pk=11, loan_id="GL-TEST-001")
        mock_get_object_or_404.return_value = template_obj
        ...
        mock_render_loan_ticket.return_value = (template_obj, b"%PDF-1.4 test")
        ...
        self.assertIn("GL-TEST-001", response["Content-Disposition"])
