...
- `django_project/tenant_urls.py` → `path("notify/", include("apps.tenant_apps.notify.urls"))`
...
- `code` → unique identifier like `LOAN_FIRST_REMINDER`
- `name` → user-facing name
- `category` → `LOAN`, `SALES`, `PURCHASE`, etc.
...
- **loan creation** → send welcome / loan-created notice
- **monthly maturity scan** → send first reminder
- **overdue escalation** → send second/final reminder
- **pre-auction workflow** → send auction notice
- **manual loan action screen** → send one-off notice from loan detail
...
- **Email** → configure SMTP / email provider
- **SMS** → configure Twilio or another gateway
- **WhatsApp** → configure Business API provider
- **Print** → ensure PDF layout in `loan_pdf.py` is correct for branch use
...
- overdue reminders
- pre-auction notices
- manual “send reminder” button on loan detail
...
4. attach the borrower’s relevant loans with `add_item()`
...
8. update status as the notice progresses (`Draft` → `Sent` → `Acknowledged`)
