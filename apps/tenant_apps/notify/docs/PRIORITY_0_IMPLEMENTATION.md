...
{% for item in items %}
- {{item.reference}}: ₹{{item.amount}} (Due: {{item.due_date}})
{% endfor %}
...
7. **Implement Scheduled Notifications**:
   ...
   - Build escalation workflows (First → Second → Final reminders)
...
✅ **Reusability**: One notification system for entire SaaS (not just loans)  
✅ **Maintainability**: Single codebase to update, no duplication  
✅ **Scalability**: Add new modules without rebuilding notification logic  
✅ **Flexibility**: Tenants can define custom notice types  
...
- ✅ 22 default notice types seeded
...
A: Go to Admin → Notify → Notice Type Configurations → Add. Set code, name, category, and templates.
