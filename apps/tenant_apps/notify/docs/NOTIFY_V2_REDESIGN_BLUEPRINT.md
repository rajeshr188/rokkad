...
1. **Event-driven** — business modules emit events; notify decides what to send.
2. **Domain-agnostic** — one notification system for Girvi, Sales, Purchase, Inventory, HR, and future apps.
3. **Channel-native** — PDF print, email, SMS, WhatsApp, and in-app are first-class channels.
4. **Asynchronous and reliable** — delivery should happen through queued jobs with retries and audit logs.
5. **Tenant-safe** — every record is tenant-scoped and operationally isolated.
6. **Operator-friendly** — templates, preview, retries, failures, and artifacts should be visible in admin/UI.
7. **Legally printable** — Girvi overdue and auction notices must support fixed PDF layouts.
...
→ resolve NotificationPolicy rows
→ create NotificationJob per selected channel
→ render NotificationArtifact
→ dispatch through channel adapter
→ store result and provider metadata
→ update audit log
```
...
→ choose “Create Reminder Batch”
→ create NotificationBatch
→ group loans by borrower or notice policy
→ create one NotificationEvent per recipient
→ create one LETTER/POST NotificationJob per recipient
→ render PDFs
→ provide combined print bundle + individual files
→ operator prints all
→ operator marks batch as POSTED_MANUALLY
```
...
A clean event → template → render → dispatch pipeline gives that without coupling the system to `GivenLoan` or any one domain.
