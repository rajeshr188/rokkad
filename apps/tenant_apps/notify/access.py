...
def assert_notify_workspace_access(request):
    try:
...
def assert_notify_permission(request, *permissions, require_all=False):
...
def assert_notify_action_permission(request, action):
    permissions = NOTIFY_ACTION_PERMISSIONS.get(action)
    if permissions is None:
        raise ValueError(f"Unknown Notify action '{action}'")
    return assert_notify_permission(request, *permissions, require_all=False)


def notify_action_required(action):
    def _decorator(view_func):
        @functools.wraps(view_func)
        @login_required
        def _wrapped_view(request, *args, **kwargs):
            assert_notify_action_permission(request, action)
...
class NotifyActionRequiredMixin(LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.required_action is None:
            raise ValueError("NotifyActionRequiredMixin requires required_action")
        assert_notify_action_permission(request, self.required_action)
