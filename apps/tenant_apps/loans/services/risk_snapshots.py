...
def refresh_loan_risk_snapshot(loan_id: int, *, as_of_date: date):
    ...
    with transaction.atomic():
        ...
        values = dict(
            ...
            assessment_fingerprint=assessment.fingerprint, input_fingerprint=after,
            status=LoanRiskSnapshot.Status.CURRENT, error_message="", assessed_at=timezone.now(),
        )
        if snapshot is None:
...
def reassess_pawn_loans_batch(*, workspace_id: int, as_of_date: date, batch_size=100):
    ...
    with transaction.atomic():
        candidate_ids = tuple(
            ...
            .exclude(risk_snapshot__status=LoanRiskSnapshot.Status.CURRENT, risk_snapshot__as_of_date__gte=as_of_date)
...
def _persist_transitions(snapshot, old, new):
    ...
    for transition in detect_risk_transitions(old, new):
        ...
        LoanRiskEvent.objects.get_or_create(
...
def _source_fingerprint(loan_id, workspace_id, as_of_date):
    loan = PawnLoan.objects.filter(pk=loan_id, workspace_id=workspace_id).values(
        "pk", "state", "loan_date", "tenure_months", "product_version_id",
        "updated_at"
    ).get()
    ...
    for name, relation in (("events", model.accounting_events.rel.related_model), ("schedules", model.repayment_schedules.rel.related_model), ("obligations", model.repayment_obligations.rel.related_model), ("allocations", model.obligation_allocations.rel.related_model), ("schedule_changes", model.repayment_schedule_changes.rel.related_model)):
        ...
        payload[name] = values
    payload["items"] = model.collateral_items.rel.related_model.objects.filter(loan_id=loan_id).aggregate(count=Count("pk"), max_pk=Max("pk"), last_update=Max("updated_at"))
    from apps.tenant_apps.loans.models import CollateralAppraisal, LoanMonitoringPolicy
    from apps.tenant_apps.rates.models import Rate
    payload["appraisals"] = CollateralAppraisal.objects.filter(collateral_item__loan_id=loan_id, effective_at__date__lte=as_of_date).aggregate(count=Count("pk"), max_pk=Max("pk"))
    payload["policies"] = LoanMonitoringPolicy.objects.filter(workspace_id=workspace_id, effective_from__lte=as_of_date).aggregate(count=Count("pk"), max_pk=Max("pk"))
    payload["rates"] = Rate.objects.filter(timestamp__date__lte=as_of_date).aggregate(count=Count("pk"), max_pk=Max("pk"), latest=Max("timestamp"))
