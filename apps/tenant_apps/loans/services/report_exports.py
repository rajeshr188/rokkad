...
def build_party_statement_dataset(statement):
    ...
    for event in statement.transactions:
        ...
        rows.append((
            ...
            event.get_event_kind_display(),
            values.get("principal", "0"),
            values.get("interest", "0"),
            values.get("fees", "0"),
            values.get("total", ""),
        ))
    return PawnLoanReportDataset(
        ...
        ("Row type", "Loan", "Date", "State / kind", "Principal", "Interest", "Fees", "Total"),
        tuple(rows),
    )
...
def _daily(report):
    rows = []
    for event in report.daily_disbursals:
        rows.append((event.effective_date, "DISBURSAL", event.loan.loan_number,
                     event.loan.borrower.display_name, (event.payload.get("values") or {}).get("principal", "0")))
    for event in report.daily_repayments:
        rows.append((event.effective_date, "REPAYMENT", event.loan.loan_number,
                     event.loan.borrower.display_name, (event.payload.get("repayment") or {}).get("amount_received", "0")))
    return PawnLoanReportDataset(
        "daily", f"Daily disbursals and repayments — {report.as_of_date}",
        ("Date", "Kind", "Loan", "Party", "Amount"), tuple(rows),
    )


def _releases_renewals(report):
    rows = [
        (row.effective_date, "RELEASE", row.loan.loan_number, row.release_number, row.settlement_amount)
        for row in report.releases
    ]
    rows.extend(
        (row.renewal_date, "RENEWAL", row.source_loan.loan_number, row.renewal_number, row.successor_loan.loan_number)
        for row in report.renewals
    )
    return PawnLoanReportDataset(
        "releases_renewals", "Releases and renewals",
        ("Date", "Kind", "Source loan", "Reference", "Settlement / successor"), tuple(rows),
    )
