...
class CollateralReappraisalForm(forms.Form):
    ...
    evidence_reference = forms.CharField(max_length=255, help_text="Inspection, report or rate-review reference; do not enter a public file URL.")
    ...
    expected_rate_id = forms.IntegerField(min_value=1, required=False, widget=forms.HiddenInput)
...
def collateral_reappraisal(request, pk, item_pk):
    ...
    form = CollateralReappraisalForm(request.POST if request.method == "POST" else None,
        initial={"expected_version": latest.version if latest else 0, "expected_rate_id": reference["rate_id"]})
