...
ALLOWED_VALUE_FORMATS = frozenset({"DEFAULT", "UPPER", "LOWER", "DATE_DMY", "DATE_MDY", "DECIMAL_2", "INR_SYMBOL"})
...
REQUIRED_BINDINGS = {
    ...
    "repayment_receipt": frozenset({"workspace.name", "workspace.source_id", "license.display", "loan.number", "borrower.display", "repayment.source_id", "document.status", "event.fingerprint", "event.effective_date", "repayment.amount_received", "amounts.fees", "amounts.total_interest", "loan.principal"}),
    "release_memo": frozenset({"workspace.name", "workspace.source_id", "license.display", "loan.number", "borrower.display", "release.source_id", "release.number", "document.status", "loan.event_source_id", "event.fingerprint", "event.effective_date", "release.type", "amounts.principal_settled", "amounts.interest_settled", "amounts.fees_settled", "amounts.total_settlement"}),
    ...
    "auction_recovery": frozenset({"workspace.name", "workspace.source_id", "license.display", "auction.source_id", "auction.number", "document.status", "event.effective_date", "auction.buyer", "auction.buyer_reference", "amounts.principal_recovered", "amounts.interest_recovered", "amounts.fees_recovered", "amounts.total_recovery"}),
    "renewal": frozenset({"workspace.name", "workspace.source_id", "license.display", "renewal.source_id", "renewal.number", "document.status", "renewal.date", "renewal.mode", "renewal.source_loan", "renewal.successor_loan", "borrower.display", "renewal.source_principal_settled", "amounts.interest_settled", "amounts.fees_settled", "renewal.principal_paid", "renewal.top_up_disbursed", "renewal.successor_principal", "renewal.successor_advance_interest", "renewal.successor_deducted_fees", "renewal.net_cash_handoff"}),
}
REQUIRED_SECTIONS = {
