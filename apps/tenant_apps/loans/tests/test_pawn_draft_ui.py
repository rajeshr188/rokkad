...
class PawnDraftUiTests(WorkspaceTestCase):
    ...
    def test_disbursal_review_uses_frozen_amounts_and_empty_post_records_nothing(self):
        ...
        self.assertContains(failed, 'href="#id_effective_date"')
        loan.refresh_from_db()
        self.assertEqual(loan.state, "APPROVED")
        self.assertEqual(loan.loan_events.count(), 0)
        detail = self.client.get(detail_url)
        self.assertTrue(detail.context["can_print_ticket"])
        self.assertFalse(detail.context["can_print_schedule"])
        self.assertContains(detail, "it is not proof of disbursal")
        self.client.post(url, {"effective_date": "2026-07-18"})
        detail = self.client.get(detail_url)
        self.assertTrue(detail.context["can_print_schedule"])
        self.assertContains(detail, "Key facts and repayment schedule")

    def test_simple_review_empty_post_is_bound_and_hindi_fields_are_accessible(self):
        ...
        self.assertContains(response, 'href="#id_confirmed"')
        self.assertNotContains(response, 'href="#id_review_token"')
        self.assertEqual(loan.approval_snapshots.count(), 0)
        self.assertEqual(loan.loan_events.count(), 0)
        self.client.cookies["django_language"] = "hi"
        response = self.client.get(url)
        self.assertContains(response, "भुगतान की तारीख")
        self.assertContains(response, "ऋण वितरण की पुष्टि करें")
        self.assertContains(response, "id_effective_date_helptext")
        self.assertIn("no-store", response.headers["Cache-Control"])
    ...
    def test_checklist_detects_missing_product_without_consuming_numbers(self):
        ...
        self.assertContains(blocked, "Choose lending products")
