...
for metal in required:
    if not Rate.objects.filter(metal_type=metal).exists():
        print(f"✗ {metal} rate missing in DB!")
```
...
if unreleased.exists():
    ...
    print("✗ No unreleased loans to analyze")
...
try:
    ...
    print("✓ manager_improved imports OK")
except ImportError as e:
    print(f"✗ manager_improved import failed: {e}")
